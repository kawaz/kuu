...
- **候補 a**: default = all と**同一の entry 集合** (全 entry + 全グループ宣言 entry)。差は renderer policy への指示だけ (L115 準拠読み)。→ DR-113 L35-37 の "all" 説明文を「default との差は renderer policy 指示」へ明確化
...
- **統括推し = a**: §4.4「hidden は model に残す。除外はレンダラ policy」の設計思想 (model は素材全部、表示選択は renderer) と一貫。b は §4.4 と正面衝突、c はグループ表示を default で組めなくなり renderer の自由を奪う

**参照**: `docs/decisions/DR-113-help-mechanism-redesign.md` §1 (L35-37) / §2 内部セル写像 (L107-115) / §4.4 (L317-318)、`schema/fixture.schema.json` L213-223 (category_mode)、`fixtures/help/absent-category.json` (書き直し中の対象 fixture)

**波及**: 裁定後 DR-113 L35-37 の文言を明確化 + absent-category.json の 3 case expect 確定。
