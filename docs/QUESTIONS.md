...
<!-- DOG-Q1: kawaz 確認 (2026-07-24) の結果、「failure-action 属性」は不正確で実在語彙は on_failure (DR-113 §7.2)。version は既存語彙 (入口 + on_failure: true + 固定文字列供給) の組み合わせで def 側に書けるため spec 増分ゼロ = 裁定不要と整理。糖衣化は実測後に issue 判断。DOG-Q2=a (help 既定 text)。 -->
