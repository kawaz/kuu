...
- **追加確定 (mid=28)**: **default_fn 一本化**、`default: value` / `env: "VAR"` / `inherit: true` は全て default_fn の糖衣 (finding §4.4 参照)。fn 名は `constant` (variant DSL `set` との衝突回避)。wire form 記法は不変、破壊的でない
