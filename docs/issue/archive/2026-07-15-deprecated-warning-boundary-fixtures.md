...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-17T17:10:30+09:00
discard_reason:
pending_reason:
close_reason: ["dr/DR-058","implemented"]
blocked_by:
...
- [x] (2)(3) の設計裁定を `docs/QUESTIONS.md` 経由で確定し、DR-058 に note を
      追記する — DEP-Q1=a (warnings は element 単位で畳む)・DEP-Q2=a (failure
      では warnings を出さない、lint/診断モードは別層) の kawaz 裁定
      (2026-07-17) を DR-058 §2 の明確化 note ×2 で確定 (commit b3cb590e)
- [x] (1)-(4) の fixture を `fixtures/alias-parse/` に追加する — (1) は
      `fixtures/alias-parse/deprecated.json` の
      `deprecated-then-canonical-mixed-still-warns-once`、(2) は同
      `deprecated-entry-repeated-warns-once`、(3) は同
      `deprecated-entry-then-failure-drops-warnings`、(4) は新設
      `fixtures/alias-parse/deprecated-variant-rederive.json` の 3 case で pin
- [x] ロックステップ (spec fixture push → pin bump → kuu.mbt push) で
      conformance green を確認する — kuu.mbt conformance 275 decoded/673
      ran/0 mismatch (CI green, commit 64ae6578)、kuu-cli 575/575 baseline
      ratchet (commit f5f9e06d)
