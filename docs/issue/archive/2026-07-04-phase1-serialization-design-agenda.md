...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-08T21:59:07+09:00
discard_reason:
pending_reason:
close_reason: ["受け入れ条件4/4達成","dr/DR-061:implemented","dr/DR-062:implemented","dr/DR-063:implemented","dr/DR-064:implemented","dr/DR-065:implemented","dr/DR-067:implemented","dr/DR-068:implemented","docs/CONFORMANCE.md:implemented","followup:2026-07-08-schema-materialization-and-reason-descriptors"]
blocked_by:
...
### C. conformance fixture フォーマット — **決着済み (2026-07-05)**

> C-1〜C-6 + error kind (parse) + effects の範囲 (cli 効果のみ、ラダー充填は result 検証) は **DR-065** として ink。現役仕様の正本は **docs/CONFORMANCE.md** に新設。最初の実物 fixture は **fixtures/dd/basic.json** (7 cases、GNU cp 実機観測との一致を why に記録)。C-7 (蒸留方針) と lowering fixture (`query: "lower"`) の詳細はフェーズ 2。以下の各節は議論経緯の記録。
...
**推奨 (2026-07-05、A-0 決着後)**: **定義は wire form (宣言層、DR-063)** で書く。A 群糖衣の展開・installer lowering の検証は C-8 の段階別 fixture が専任で担うので、parse fixture は評価器 (+決定的 lowering) の検証に純化する — 責務が重複しない。糖衣込み (素の UsefulAST) を parse fixture に混ぜると「糖衣展開の差」と「評価の差」が 1 つの fail に混ざり切り分けが濁る。
...
**推奨 (2026-07-05)**: **(b) 素の JSON + `why` 予約フィールド**を fixture スキーマの正式な一部にする (file / case の各レベル。DR 根拠は文中に書く)。(a) JSON5/JSONC は「各言語の JSON パーサがそのまま読める」制約を破る (パーサ依存が増える)。(c) 隣接 .md は fixture と意図が物理分離して乖離する — 「意図コメントは fixture データに同梱」(ROADMAP、テスト = 真の仕様書) に反する。`why` はコメント構文でなくデータなので、runner がレポートに表示する・lint が「why なし case」を検出する等の機械利用もできる。
...
**推奨 (2026-07-05)**: **(b) + 拡張点の明示** — fixture のトップレベルに `query` 種別タグ (`"parse"` を既定) を置き、parse フォーマットを先行確定する。フェーズ 2 の蒸留対象 (slice 167 テスト) は parse が主なので、complete (候補構造の直列形確定が必要) と definition-error を今固めるのはフェーズ 1 の荷物を増やすだけ。タグがあるので後続追加はフォーマット破壊にならない。
...
**推奨 (2026-07-05)**: **機能領域別** (`fixtures/repeat/` / `fixtures/dd/` / `fixtures/constraints/` 等)。1 つの挙動は複数 DR の合成で決まる (例: dd の挙動 = DR-041 + DR-042) ため DR 番号ベースは置き場が一意に決まらない。DR への遡及は各 case の `why` フィールド内の DR ref で辿る。段階別 fixture (C-8) は `fixtures/lowering/<installer>/` の別枝。
