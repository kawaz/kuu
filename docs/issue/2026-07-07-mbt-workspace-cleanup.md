...
- [x] workspace forget の実行 (2026-07-12 kawaz 承認、実施済み)
  - `jj workspace forget refactor review parts-arggen` (kuu.mbt/main から) 実行済み
  - `refactor` の未コミット変更は保存せず破棄 (jj op log からは復元可能)
  - ディレクトリ実体の削除は未実施、kawaz 手動実行待ち
- [ ] origin 残存枝の削除 (**kawaz 確認後**、kawaz 指示により「また今度」で保留中)
  - `claude/review-implementation-gLfMA` / dependabot 2 枝を origin から削除
- [ ] bookmark は削除しない (= `kuu-v0` 等のデータ保護。到達性維持)

## 対象外 (温存)

`kuu.mbt/ast-spec` / `kuu.mbt/divfix` / `kuu.mbt/slice` workspace は本 issue の対象外。
現行の参照実装作業用として温存する。
