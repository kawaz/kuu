...
last_read:
open_entered: 2026-07-14T23:45:07+09:00
wip_entered:
blocked_entered:
pending_entered:
discarded_entered:
...
## 動機の格上げ (2026-07-14 追記)

`docs/VISION.md` §4 (2026-07-14 新設、「独自フィルタの可搬性 — descriptor
による型・モック生成」) により、本 issue が扱う descriptor の
invocation/型シグネチャ宣言軸は **lint/doc 用 metadata に留まらない**ことが
判明した。VISION §4 が示す 4 段フロー:

1. 独自フィルタが自身の descriptor (入出力の型、`signature`、`reasons` 等) を宣言する
2. export した定義 JSON にその descriptor (シグネチャのみ) を同梱する
3. import 先の言語では descriptor から interface / struct 定義を生成し、
   型を当てたモックコードまで出力する
4. 開発者はその言語の型エラーに従って、足りない実装だけを埋めれば動く

このフローが成立するには、`config` 欄に DSL 引数の意味を注記として同居させて
いる現状の invocation 軸の歪み (上記「背景」節 2.) を解消し、独立した宣言軸に
昇格させる必要がある。つまり本 issue の軸分離作業は、多言語可搬性
(幻影コマンド体験の限界緩和、VISION §3「幻影コマンド体験」参照) を実現する
**中核機能**の前提条件になる。

軸再整理の検討 (受け入れ条件の DR 起票時) では、VISION §4 の 4 段フローを
要件入力に含めること (= invocation 軸の宣言形式が上記 2〜4 を機械的に
駆動できる粒度・情報を持つか確認する)。

## 着手順序 (kawaz 方針 2026-07-14 r14)

kuu-core API 整理 (kuu.mbt リポ `docs/issue/2026-07-14-kuu-core-front-door-api.md`)
の完了を待ってから本 issue に着手する。
...
- `docs/VISION.md` §4 (独自フィルタの可搬性 — descriptor による型・モック生成)
- kuu.mbt リポ `docs/issue/2026-07-14-kuu-core-front-door-api.md` (先行着手対象)
