...
関連: A-0 ③ (wire 追加語彙の許容範囲の機械判定基盤)、DR-054 (unknown-vocab)、DR-056 (所有/参照)、§13.7 (diagnose)。
...
- [ ] A群 (wire form 範囲 (A-0) / greedy マーク / matcher データ / 効果記述子 / repeat 匿名id / 一意性 / pending 状態 / installer descriptor (A-7)) の各論点に決定が付き、新規 DR (DR-061 以降を想定) または既存 DR 拡張として記録される
