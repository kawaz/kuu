...
#### A-7. installer descriptor — 所有語彙 + 観測語彙の宣言形式化 (2026-07-04 追加)
...
- 観測側のフィールド名: (a) `references` (DR-056 の「参照」をそのまま写す、新語ゼロ) / (b) `observes` (「観測のみ・効果列に影響不可」の制約が名前に乗る。ref/link/alias の参照ファミリーとの語の衝突も避けられる)。「依存 (depends)」は不採用方向 — 順序制約を連想させ、不動点の順序非依存 (参照は宣言層を読むので順序問題は原理上出ない) と誤読が衝突する
...
#### A-8. registry 住人の configurable factory 化 (2026-07-04 追加、kawaz 提案)
...
- **config スキーマの宣言形**: descriptor が「この factory はどのキーをどの型で受けるか」を宣言する = mini-schema が registry 住人ごとに再帰する (B 群と同根)。初手は「キー名 + 型 + default の平坦リスト」に抑えたいが、ネスト config (base_prefix の map 形) が既に例に出ており平坦で収まるか要確認
- **pre_filter との線引き**: DR-040 の 2 軸のもう片方 (寛容 default + pre_filter) と同じ結果を 2 経路で書ける redundancy が生まれる。指針候補: 「字句の受理域を定めるのは factory config、値の変換・検証は filter」。曖昧なままだと方言の書き方が実装間でブレて conformance が濁る
