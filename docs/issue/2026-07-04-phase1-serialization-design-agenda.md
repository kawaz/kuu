...
DR-060 の補完クエリで観測された「parse の枝列挙が『トリガ消費・値pending』を中間状態として表現するか」という設計軸 (slice PoC 第18弾で実測、鏡写し専用走査で分離可能なことが確認済み)。通常パースの評価器が pending 状態を明示的な中間ノードとして持つ設計を採るなら、A-1/A-2 の直列形にも影響しうる (matcher が「トリガは消費したが値はまだ」という状態を表現する必要が出るか)。DR-060 は「実装機構の共有は要求しない」としているため、AtomicAST 直列形自体が pending を持つ必要はない可能性が高いが、A-2 のエントリ表設計時に確認しておくべき前提。
...
- byte 厳密比較が必須なケース (exact 照合の Unicode 正規化 F-006/DR-047、canonical default 数値表現 DR-040 の字句仕様) は fixture 側で明示タグ付けし、それ以外は意味論比較に倒す、という DR-040 の「canonical/標準/拡張3層」構造との接続方針も要確認
...
#### C-5. fixture のディレクトリ構成・命名規則

DR 番号ベース (`fixtures/dr-038-path-search/*.json`) か機能領域別 (`fixtures/multiple/*.json`) か。C-1 の definition 形式・C-6 の蒸留単位と整合させる必要がある。

#### C-6. 蒸留元 (slice 167テスト) からの変換方針

slice PoC の 167 テスト (kuu.mbt slice 枝、フェーズ2 の蒸留元) を 1:1 で fixture 化するか、意味論単位 (DR 単位 / 領域単位) で再編するかは本フェーズでなくフェーズ2の主題だが、C-1/C-5 の設計がフェーズ2の変換コストを左右するため、フェーズ1側で「蒸留のしやすさ」を設計基準に含めるかどうかは論点として明記しておく。
