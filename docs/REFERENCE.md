...
> 機械検査の対象は wire 正規形の語彙のみ — UsefulAST 専用の糖衣属性 (`values` / `hidden` 等、
> 正本上 wire の properties に持たないため機械検査は対象外) も定義 JSON を書く上では必須語彙
> なので、§2.3 に手動転記のカタログとして掲載する。
...
| `help_group_name` | string | なし | options/commands の entry / グループ宣言エントリ |
...
正本: DESIGN §15.10, DR-048, DR-112 §8

**`help_on_failure`**
型: boolean。既定: true。適用対象: `type:"help"` 系要素 (help / help_all / help_category) 専用。
意味論: type config 糖衣 — 汎用属性 `on_failure` へ全展開される。false で失敗時発火を切る。
type:help 系以外の要素への宣言は definition-error (kind: `invalid-range`)。
最小例: `{"name": "help", "type": "help", "long": true, "help_on_failure": false}`
正本: DR-112 §8, LOWERING §A.5

#### 表示メタ (help_meta installer 所有)

宣言層の inert 属性群 — パース挙動に影響せず、help query (DESIGN §15.15) が参照で読む。
「AtomicAST 非搬送」(DR-046 §3) は「lowered 産物・評価器へ運ばない」の意で、宣言層 (wire) には載る
(DR-112 §1)。
...
正本: DESIGN §1.4, §2.2, §14.6, DR-046 §3, DR-112 §4
...
正本: DESIGN §14.6, DR-112 §4

**`help_epilog`**
型: string。適用対象: スコープ要素 (ルート / command)。
意味論: オプション一覧の後に出す自由テキスト素材 (連絡先・注意事項・例の手書き等)。葉要素に
付いても合法だが v1 の model に座席が無い (lint warn の関心)。
...
型: いずれも string。適用対象: options/commands の entry (所属参照) / グループ宣言エントリ (宣言)。
意味論: 表示グループ。`name`/`id`/`type`/入口系属性をいずれも持たない entry に `help_group_name`
を書くと**グループ宣言エントリ** — グループの表示順 (= 宣言順) と表示メタ (title / description) が
一箇所で完結する。それ以外の `help_group_name` は所属参照。同名グループへの別設定の重複宣言は
definition-error (kind: `invalid-range`)。
最小例: `{"help_group_name": "net", "help_group_title": "Network options"}` を `options[]` の先頭に
置き、メンバー要素に `"help_group_name": "net"` を付ける。
正本: DESIGN §14.6, DR-112 §5
...
(`values`)、または所有座席が未実体化のままの語彙 (`hidden`、DR-112 §9) のため **`schema/
wire.schema.json` の `$defs.node.properties` に持たない** — additionalProperties が開いているため
構文検査は通るが機械的な網羅性検査 (§2.1 の `just lint-reference`) の対象にはできない。**本表が
これらの属性の唯一のカタログ**であり、audience (定義 JSON を書くアプリ開発者) にとっては
§2.1/§2.2 と同格の必須語彙。表示メタ語彙 (`help` / `help_long` / `help_epilog` / `display_name` /
`value_name` / グループ・順序語彙) は DR-112 §1 の明確化 (宣言層には載る) により wire 正規形へ
収載済み — §2.1/§2.2 を参照。
...
| `help` | 糖衣プリセット | help 内部セルへの link + 固定 true 供給 (失敗時も候補経路で selected なら発火、`help_on_failure` 同梱) | DESIGN §3.3, §14.1, DR-112 §7 |
| `help_all` | 糖衣プリセット | help と同族 + 「hidden 込み全表示」の意図メタ | DESIGN §14.1, DR-112 §7 |
| `help_category` | 糖衣プリセット | string カテゴリセル + help 内部 bool セルの同時トリガ。bool 枝との出し分けは `or` | DESIGN §14.1, DR-112 §7 |
