...
**参照側は写像を意識しなくてよい** — 参照属性 (`ref` / `link` / `borrow` / `requires` / `conflicts_with` の 5 つ。DR-136 §6 の対象はこの閉じた集合で「等」で開かない) と link
...
はこちら。並べるのは要素への参照ではなく**グループラベル**なので、2 段ルックアップ (DR-136 §6) の
対象外 — ラベルどうしは raw の文字列一致だけで突き合わせる (`required_group` も同じ)。
...
| `config_file` | 特殊 type | config ファイルパスの配線宣言。**1 スコープ 1 要素** (並置は definition-error `invalid-range`)、その要素が供給するパスを供給順に読み、トップキー置換で畳んだ 1 個が config 席を埋める (重ねは `multiple`、単値は列長 1 の縮退形)。要素自身は**通常要素**で、露出キーがあれば確定したパスが `result` / `sources` に座り露出キー衝突検査にも参加する (隠すなら `export_key: null`) | DESIGN §14.3, DR-050/133/135 |
