...
| キー | 型 | 既定値 | 適用対象 |
|---|---|---|---|
| `accum_filters` | filterChain (array \| {prepend?,append?}) | 空 | multiple 要素専用 (DR-102) |
...
| `final_filters` | filterChain (array \| {prepend?,append?}) | 空 | multiple 無し値要素専用 (DR-102) |
...
**`final_filters`**
確定した最終値への T→T チェイン (multiple 宣言のない値要素専用)。multiple 宣言のある要素への宣言は
definition-error kind=invalid-range。
最小例: `{"name": "verbose", "type": "count", "final_filters": ["in_range:0:2"]}`
正本: DESIGN §8.3, DR-102

**`accum_filters`**
累積後 (accumulator 後) の Acc→Acc チェイン (multiple 宣言のある値要素専用、cell 単位)。multiple
宣言のない要素への宣言は definition-error kind=invalid-range。
