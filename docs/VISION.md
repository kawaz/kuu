...
kuu-cli の存在を前提にすると、アプリ内に kuu ロジックのバンドルを持たない「ほぼ wrapper のみ」の言語 UX を用意できる。アプリ本体は definition JSON の export と kuu-cli 呼び出しの薄いラッパーに留め、実際のパース処理は外部の kuu-cli プロセスに委譲する構成である。

多言語展開の最初の一歩は、各言語のネイティブ実装 (fixture pass による移植、§2 の各言語 kuu-core) を用意せずとも、この極小バンドルモードを用意するだけで最低限実現できてしまう — 言語ネイティブ実装 (fixture pass) よりはるかに安い展開経路になる (DR-109 §6)。

kuu-cli がアプリの実行経路そのものになるこのモードでは、kuu-cli は開発補助のテストツールではなく本番の実行主体を兼ねる。したがって kuu-cli はアプリに組み込まれた kuu 実装と同一の definition で同一に振る舞うことが本義であり、既定で実環境 (env・config・tty) を取得する契約になる — 試験用の環境固定・注入オプション (`--no-env` / `--env k=v` 等) は別途用意する (DR-109 §6)。
...
### RPC によるクロージャ注入 (第 3 の注入口、構想・未確定)

独自フィルタのクロージャをホスト側 (本体アプリ) に保持したまま、kuu-cli プロセスに処理を委譲する第 3 の経路として、本体アプリと kuu-cli の間でクロージャ呼び出しをやり取りする RPC spec の構想がある。これが実現すれば、独自フィルタの注入口は以下の 3 系統になる:

1. **言語実装テンプレ** (capability marker + codegen、本節冒頭の descriptor 由来コード生成): 各言語のネイティブ実装がクロージャをホスト言語のコードとして持つ王道経路
2. **WASM embed** (上記): フィルタ実装そのものを definition JSON に同梱し import 先でそのまま実行
3. **RPC によるクロージャ注入**: 本体アプリがクロージャを保持したまま、kuu-cli からの呼び出しに RPC で応答する

RPC 経路が確立すれば、極小バンドルモード (§3) と組み合わせて、「極小 wrapper でありながらクロージャ対応まで可能」な「RPC 付き 2nd 最小バンドル」オプションが取れる (DR-109 §6)。RPC spec 自体は未設計であり、確定した設計判断は無い。
