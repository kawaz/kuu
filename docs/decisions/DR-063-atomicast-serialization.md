...
> **更新 (DR-140 §5、2026-08-16): §3 の最小投影フィールド名が参照する「仕様語彙」は
> DR-139/140 の値カプセル語彙に更新される。** 断面がフラットな最小投影であることは不変
> (wire の入れ子はなぞらない)。
...
- **templates 面 (5 面目)**: ref 再帰 (repeat の cons) や共有 template の置き場。断面は `{greedy, positionals, entities, constraints, templates}` の 5 面で、空の面は省略可 (最小投影)。**templates のキーは常に `#` 予約名前空間の内部名** (例: `src#cons`。lowering 産物の匿名構造でありユーザの name ではない。綴りは非規範で緩比較が吸収)。プレーン名を使わない理由: entity と同名になると、cons の head が使う `{ref: "<実体名>"}` (entities への構造継承) と template 自己参照が**曖昧になる** — `#` 修飾により ref の解決先が常に一意 (`#` 入り = templates、なし = entities/要素)。inline 展開を採らない根拠: 共有 template の ref site ごとの複製は「実体複製が link 合流の同一性を壊す」(matcher entries と同型) 問題を起こし、自己再帰はそもそも inline 不可能
- **golden は正規形で書く**: 断面 fixture の消費点 (greedy 衛星の値スロットも背骨の消費点も) は ref + link の宣言形で統一する。inline 解決済み形 (値プリミティブ直埋め) を緩比較が同一骨格として受理するのは**実装出力の観測側**の話であり、golden 自体は正規形 (DR-070 §1b の「仕様準拠値」原則の表記面)。entities の最小投影フィールド名は仕様語彙 (`type` / `default` / `accumulator` / 席宣言) を使う — 実装内部の略名 (accum 等) を持ち込まない
- **入れ子 scope は再帰的に同じ 5 面構造**: command 部分木の子 scope も `{greedy, positionals, entities, constraints, templates}` を持つ (空の面は省略)。attach 表記は **greedy のトリガ衛星が scope 面を抱える形** `{"exact": "serve", "scope": {...子の 5 面...}}` — command は親 entities に実体を持たない (スコープ名はトリガ衛星の exact 綴りとして配置で表現される。結果オブジェクトの `{serve: {}}` presence は実行時 result builder の関心であって断面の関心ではない)
