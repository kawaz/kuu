...
- **templates 面 (5 面目)**: ref 再帰 (repeat の cons) や共有 template の置き場。断面は `{greedy, positionals, entities, constraints, templates}` の 5 面で、空の面は省略可 (最小投影)。templates のキーはプレーン名 (= 元要素名。min≥2 の匿名尾部のみ `#` 予約 id) で、entity と同名共存可 (別名前空間、名前が join キー)。背骨からは `{ref: "<名前>"}` で指す。inline 展開を採らない根拠: 共有 template の ref site ごとの複製は「実体複製が link 合流の同一性を壊す」(matcher entries と同型) 問題を起こし、自己再帰はそもそも inline 不可能
