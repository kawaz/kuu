...
- **multiple (値の畳み)**: option の複数回発火や separator 分割片が生む値列を畳む DR-034 のパイプライン (separator / accumulator / collector)。**出現回数・出現位置の制約は持たない** (回数は repeat、位置の自由は greedy の軸)。
...
- **repeat installer**: `repeat` 属性を回収し、**ref を使った再帰リスト構造に lowering する**:
  ...
  (`color` を `[r,g,b] | colorname` のような構造テンプレとして export_key: null (DR-052) で定義し bg / fg から ref する使い方の延長)。head の消費は ref 先の構造が担うため、ゼロ進捗ガードは「ref 先が 1 トークン以上消費すること」の検査に帰着する。

  平坦化 ([T,[T,…]] → T[]) の accumulator は組み込み名 **`flatten`** として accumulators registry (DR-036) に登録する。また **`optional: true` は `repeat: {min: 0, max: 1}` の糖衣**であり、閉包に還元される (独立概念ではない)。

  > **更新 (DR-105): `flatten` の独立 accumulator エントリは廃止され、`append` accumulator の `flatten: true` ダイヤルに統合された。repeat installer の cons 平坦化は `{"accumulator": "append", "flatten": true}` を src の値セルへインストールする形に追従する。「組み込み名 `flatten` として登録する」という上記本文の記述は現役仕様ではなくなったが、判断経緯 (repeat cons の平坦化という用途自体) は不変。**
...
帰結: 「上限なし閉包が positional 列に複数 → 潜在 ambiguous」という静的 warn は不要になり、DESIGN §15.6 (静的バリデータ warn/lint, DR-021) から当該項目を削除する (§15.6 自体は他の warn/lint を持つ現役の節として存続)。

### 取り分の「向き」= 左先・最長で確定

greedy の「長い取り分から」が同一列内でどちらの閉包を優先的に最長にするかは、**左先・最長** (先の閉包から最長を試す、`[a,b,c,d],[e]`) で確定する (kawaz 裁定 2026-07-08、当初の暫定固定を本決め)。

「取り分次元は選好で代表 1 本に確定する (ambiguous にしない)」という骨子はこの向きの上に立つ。`fixtures/path-search/ambiguous-receptacles` の result 期待値 (`xs` 最長側) はこの確定に基づく正式な輪郭である。

### required との関係

repeat の min は閉包の反復回数 (CLI トークン消費の構造) であり、required (値の充足、判定は DR-047) とは別概念として立てる。両者は重ならない。
