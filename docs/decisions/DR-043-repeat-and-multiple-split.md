...
greedy の「長い取り分から」が同一列内で **どちらの閉包を優先的に最長にするか** (左=先の閉包を最長にするか、右=後の閉包を最長にするか) は、本 DR では上例のように **左先・最長** (`[a,b,c,d],[e]`) で暫定固定するが、この向き自体は完全には決着していない (再燃余地あり)。

ただし向きが反転しても **「取り分次元は選好で代表 1 本に確定する (ambiguous にしない)」という骨子は不変**。変わるのは確定した 1 本の result がどちらの閉包に寄るか (`xs` 側か `ys` 側か) だけである。したがって向きの議論が再燃・反転した場合、影響を受けるのは配分を固定した fixture (例: `fixtures/path-search/ambiguous-receptacles` は現在 `xs` 最長側に寄せている) の result 期待値であり、そこが不整合を起こすことで再是正議論の起点になる想定。この fixture の各 case が success 1 本になること自体は向きに依存しない確定事項だが、寄せ先は暫定である。
