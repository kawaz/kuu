...
> **明確化 (統括検証 2026-07-15、MISC-Q1=a 確定 2026-07-22): origin は候補が示す消費点の名前を伝播する。** (i) **global (Rooted 衛星) 越し**の候補の origin は、コピー先の子 scope ではなく宣言元 canonical の**参照識別子 (id、TRG-Q1=b)** ((c) の alias と同じ帰結。`fixtures/complete/global-scope-union.json` で pin)。 (ii) **repeat lowering の内部 id** (`#` 予約名前空間、DR-046 §4) は origin に現れない — lowering 産物の内部でも primitive の name は元の要素名のまま伝播する (`fixtures/complete/repeat-internal-id-origin.json` で pin)。 (iii) **DR-063 A.1 の裸文字列→exact 正規化で生まれる匿名 exact 候補**の origin はその spelling 自身 — 名前を持たない要素の fallback (`fixtures/complete/anonymous-exact-origin.json` で pin)。 (iv) **ref template (DR-078) 越し**では、trigger 候補の origin は入口を宣言した参照元要素名、値位置候補の origin は template 内部 leaf 名とする。この非対称は trigger が参照元 entry、値位置が実際に値を受け取る leaf を示すためのもの。`fixtures/complete/ref-template-origin-trigger.json` が trigger 側を、`fixtures/complete/ref-template-origin-value.json` が値位置側と §3 の 6 フィールド identity による非 dedup を pin する。
