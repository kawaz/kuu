...
### 3. フィールドは全列挙し、値の無い座を `null` にする

record は closed なキー語彙を持つため、結果射影では宣言済みフィールドをすべて列挙し、値が確定しない座を `null` にする (DR-130 §4)。type パーサの生出力は宣言済みフィールドを省略してよく、射影層が不足フィールドを `null` で補う。値空間側で record を読む相も、宣言済みだが物理的に欠けているフィールドを論理的な `null` として扱う。

**closed はキー語彙の閉域であって全フィールドが非 null という意味ではない。** 「どのキーが現れうるか」は宣言で閉じ、各座に値があるかは値ごとに変わる。

言語バインディングの型導出 (DR-130 §7) は record の内側にも降り、全フィールドが `T | null` になる。**値セルの型導出の正本は `io_type.output` の record 宣言だけである** (kawaz 裁定 2026-08-01)。型が入力定義片 (`input_structure`、research 2026-07-31) を持っていても、定義片は CLI トークンの消費文法を語る入力側の機構であって型導出には関与しない — 定義片 leaf の `required` / `default` から `T` を導く経路は設けない。
...
乖離検査は射影で null 補形する前の type パーサ生出力に対して行い、次の 3 状態を区別する:

| 乖離 | 扱い |
|---|---|
| (a) 宣言に無いキーが値に存在する | Error |
| (b) 宣言済みキーの値が**フィールドの type が名乗る `out`** と合わない | Error |
| (c) 宣言済みキーが値に存在しない | **正常** — 射影層が `null` を補う (§3) |
...
### closed / null 補形 / Error は 1 つの姿勢の 3 面

3 つの裁定はいずれも「type パーサは自分が返す値の形を名乗り、名乗った内容に責任を持つ」という同じ姿勢から出ている。closed でなければ名乗りは値を尽くさず (直訳できない)、宣言済み座を null で補わなければ成功 result のキー集合が実行ごとに揺れ、乖離を Reject にすれば名乗り違反がユーザ側の失敗として偽装される。
...
- **DR-130**: §4/§7 の全フィールド列挙 + null 射影と `T | null` 型導出が record の内側に適用される。フィールドの型は参照先 type の `out` を辿って求め、射影層が type パーサ生出力の欠落フィールドを補う
...
- DR-130 (成功 result の全キー列挙・record の null 補形・`T | null` 型導出)
