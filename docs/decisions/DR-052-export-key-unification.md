...
> **更新 (2026-08-16): §15.5 の評価「変更なし」は現行では成り立たない。** 露出規則と
> スコープ生成が TRG-Q4=a で結果キー軸の占有に統一されたため、透過の判定は本 DR の
> `export_key: null` と同一軸になった (影響が無いのではなく、同じ軸に畳まれた)。

## 関連

- DR-046 (名前の軸分解 — 結果キー軸のフィールドが本 DR で確定、射程外の解消)
- DR-025 / DR-033 (露出規則・lexical スコープ — 透過の導出元。**スコープ生成の条件は TRG-Q4=a で「解決後の結果キー軸の占有」に訂正された** — 本 DR の `export_key: null` = 透過はその判定と同一軸であり、結果スコープと lexical スコープの両方が消える)
