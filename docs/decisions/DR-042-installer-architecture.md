...
> **更新 (DR-112、2026-07-18): `on_failure` installer (所有語彙 `on_failure` — 失敗時発火マーカー能力の宣言、constraint と同型の構造衛星なし。DR-112 §8) と `help_meta` installer (所有語彙 help / help_long / help_epilog / display_name / value_name / help_group_* / help_order / help_after — lowering 寄与ゼロの純所有 + 定義時検査。DR-112 §9) が canonical セットに加わった。**
...
- DR-064 (dd の宣言配置 — 本 DR の展開パターン例にある dd の positionals[] 配置は canonical でなくなった。canonical は options[]、配置は挙動不問)
