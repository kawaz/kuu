...
slice PoC で確認済み: canonical 4 installer の全 24 順列一致・1 要素全部盛り・dd 再スコープ・冪等 (第 1 弾、21/21 pass)、**7 installer の全 5040 順列一致・非削除①' + 不動点反復・global × dd・多段 global 伝播と中間 shadowing・repeat 再帰と result builder の整合** (第 2 弾、45/45 pass)。未実施: repeat の max 上限 / separator 併用 (`prog a,b c,d`)、export_key / export:false と result builder、3 段以上の中間 shadowing の name-vs-trigger 微差、inheritable × long、env × config_key。
