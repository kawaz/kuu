...
3. **実行時能力の提供**: 必要なら native matcher (トークン読み、DR-041) や値源 lookup を評価器に登録する
...
5. **スコープ境界は greedy 面を断つ**: 入れ子 command で必要な規則 (`git --git-dir X commit` の後で git 側 options は効かない) を dd の option 抑制にも再利用する。dd installer は継続を子スコープに包むだけで、順序非依存に抑制が出る。`global: true` (findings F-007) はこの規則の**例外を作る側**の installer として将来定義する。
...
- **short installer**: `short` を回収し、greedy 面に native matcher (cluster / 値付着の読み生成、DR-041 §3-4) を追加。回収したエントリ表 `{p: port, v: version}` が matcher の構成データ
- **env installer**: `env` を回収し、port の env 席に lookup を宣言 (不変則④)
- **dd installer**: `type: "dd"` を回収し、継続 (dir 以降) を greedy 面を断った子スコープに包んだ or 構造へ書き換え (不変則⑤)
...
|---|---|---|
| `long` | `long` 属性 (variant DSL の値語彙 DR-011 を含む) | greedy 衛星 + eq-split matcher。config `long_prefix` / `allow_equal_separator` がパラメータ |
| `short` | `short` 属性 | greedy 衛星 + cluster / 値付着 matcher。config `short_prefix` / `short_combine` がパラメータ |
| `dd` | `type: "dd"` | 再スコープ化構造 (matcher なし) |
...
- **座席**: matcher は **greedy 面のエントリ** (exact 衛星と同列の住人)。filter 席でも専用席でもない。先食い判定 (DR-041 §4) が exact 衛星と matcher を一様に扱えることがこの座席を正当化する
- **表現**: matcher はクロージャではなく**名前付きデータ** (matcher 種別 + 回収エントリ表)。lowering 後の全体が比較・直列化可能になり、installer 順列テストの構造比較と AtomicAST のシリアライズ可能性の両方をこれが支える
- **installer の 2 類型**: long / short は**構造構築型** (衛星 + matcher を植える)、dd / env は**語彙 licensing 型** (語彙を回収して席宣言やマーカー lowering に委ね、意味論は評価器側の規則 = 不変則⑤等が担う)
