...
方言 (Go 風単ダッシュ long 等) は拡張 installer または canonical installer のパラメータ差しで提供する。`repeat` / `multiple` は DR-043、`config` (config_key / type: "config_file") は DR-050 で canonical セットに加わった。将来の候補: inheritable の prefix 生成 (DR-013 後継)。
