...
>
> **更新5 (DNR-Q1=a、kawaz 裁定 2026-08-14 / 枠付けは kawaz 裁定 2026-08-15 で訂正): `duplicate-id` を追加。** 同一 lexical スコープに同じ**参照識別子 (id)** を持つ要素が 2 つ以上ある定義。エラーの正体は **id 軸の重複そのもの** — ref / link の解決対象が一意に定まらない (DR-032) からである。既存 kind に相乗りさせない: `invalid-range` は 1 要素内の属性の組合せが値域外であることの申告で別要素間の衝突を表せず、`export-key-collision` は露出キー軸の判定だからである。
>
...
> - **参加する要素**: 同一 lexical スコープ (DR-025 / DR-033 — **解決後の結果キー軸を占有するノード**が作る、TRG-Q4=a) にある、参照識別子を持つ宣言要素。**command も参加する** (CMDID-Q1=a、kawaz 裁定 2026-08-15) — command は実体を持ち id 軸を占有するので、同名 2 本が `id` 未指定なら参照識別子が重複する。値セルの有無や露出キーの占有 (DR-120 §4) は参加条件ではない — 非占有要素 (`export_key: null` の `config_file` 等) も参照識別子は持つ
> - **スコープを作らない段を挟んでも同じスコープ**: 結果キー軸を占有しないノード — 匿名の `or` / `seq` や、`export_key: null` を書いた要素 (**command を含む**) — はスコープを作らない (DR-025 / DR-033、TRG-Q4=a) ので、その **named 子は囲みスコープの兄弟として参加する**。逆に言えば子 scope を作るのは**結果キー軸を占有する command** であって、command であること自体ではない。`options: [{"name":"a"}, {"or":[{"name":"a"}, ...]}]` は 2 つの `a` が同一 lexical スコープに居る形であり `duplicate-id` が立つ (`fixtures/definition-error/duplicate-id-anonymous-structural.json`)
