...
- **フォーマット (TOML / YAML / JSON)・探索ルール・複数ファイルのマージ戦略は provider の関心**。kuu core はフォーマットを知らず、JSON 同型の階層オブジェクトだけを見る。デフォルト実装のフォーマット選定は言語 DX の関心
  - > **読み直し (DR-133 §6、2026-08-12)**: この「複数ファイルのマージ戦略」は **1 つのパス指定から provider が探索して合成する類**を指す。**定義が宣言した複数パス** (複数の config_file 要素 / `multiple` の複数供給) の fold は core の席埋め規則であり、正本は DR-133 (パス列をトップレベルキー置換で畳む)。provider 契約は 1 パス → 1 オブジェクトのまま無傷で、core は列の長さだけ provider を呼ぶ
