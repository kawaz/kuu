...
- **フォーマット (TOML / YAML / JSON)・探索ルール・複数ファイルのマージ戦略は provider の関心**。kuu core はフォーマットを知らず、JSON 同型の階層オブジェクトだけを見る。デフォルト実装のフォーマット選定は言語 DX の関心
  - > **読み直し (DR-133 §7、2026-08-12 / 2026-08-14 再改稿)**: この「複数ファイルのマージ戦略」は **1 つのパス指定から provider が探索して合成する類**を指す。**定義が宣言した複数パス** (`multiple` の複数供給) の fold は core の席埋め規則であり、正本は DR-133 (パス列をトップレベルキー置換で畳む)。provider 契約は 1 パス → 1 オブジェクトのまま無傷で、core は列の長さだけ provider を呼ぶ。なお**同一スコープに config_file 要素を 2 つ以上置くことはできない** (DR-133 §1、definition-error `invalid-range`) ため、複数パスは常に 1 要素の `multiple` から来る
...
- 複数の config_file 要素の合成 (システム設定 + ユーザ設定の 2 要素等) は本 DR では確定しない。canonical は単一の config_file 要素
  - > **確定 (DR-133 §1、2026-08-14)**: 単一の config_file 要素が canonical であるだけでなく、**同一スコープの並置は definition-error** になった。システム設定 + ユーザ設定の重ねは 1 要素の `multiple` (配列 `default` で既定パス列) で書く
