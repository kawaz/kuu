...
- **非 string (number / bool)** → 型一致検証 + post_filters のみ。pre_filters / parse は String → 型の関数なので、既に型を持つ値には適用対象が無い — スキップは特別規則ではなく型の帰結
- **寛容 canonical**: number 要素に JSON string `"8080"` が来たら parse を試みて受理する (DR-040 の「canonical は最も寛容」)。狭めたい場合は方言 / pre_filter
...
- 型不一致 (bool 要素に number 等、parse も不能) → DR-037 の **Error** (この値源のつもりだが値が不正)
...
### config 値を全て文字列化して peaceProcessor に流す
