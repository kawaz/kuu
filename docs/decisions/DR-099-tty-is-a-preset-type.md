...
> **更新 (DR-125、2026-07-29): 値源ラダーから `inherit` 席が抜けた。** §1 が復元対象として引用する
> ラダーは 4 段 (CLI/link > env > config > default) で、引用中の「4. inherit (祖先 scope)」は対象を
> 失っている。§2 が DR-098 §5 から引く優先関係も「明示 (CLI/env/config) > 観測 (tty) > 宣言既定
> (default)」となり、tty 観測を型の default 解決規則として吸収する本 DR の決定 (独立席を持たない)
