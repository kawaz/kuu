...
{"exact": "--verbose", "link": "verbose", "effect": {"op": "update", "transform": "increment", "args": []}}   // 効果記述子 (DR-045)
```
...
- 組み込み transform として **`increment`** (number: `(old) => (old ?? default) + 1`、0-arg) を filters registry に置く

### 3. count は「number + default:0 + `update:increment` の綴り合成」に正規化する
...
{type:"count", long:true}    ≡ {type:"number", default:0, long:[":update:increment"]}     // 裸発火で +1
```

- **糖衣差し替え**: count の `long:true` 糖衣 = `[":update:increment"]` (他の型の `[":set"]` を preset が差し替える — flag の `[":set:true"]` と同じ機構、DR-076 §2)
...
parser は全値源 (CLI トークン / DSL literal / env / config) が共有する字句層 (DR-074/075) で、old 依存にすると env の `VERBOSITY=5` が increment になる等のねじれと、「畳みの無い値源での old の定義」「bare 発火の sentinel 文字列」という追加規約 2 つを払うことになる。fold は発火側 (CLI 面固有の関心) に置く。
