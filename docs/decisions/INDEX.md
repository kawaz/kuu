...
- [DR-076](DR-076-bool-is-value-type-flag-is-sugar.md): bool は通常の値型 (variant 持ちでも eq-split 値形を受ける)、flag/count が特殊 — 特殊性は flag/count 側の展開 (bool への糖衣、long 綴り合成 or Exact+link) で吸収。presence-only へ寄せる案を不採用
