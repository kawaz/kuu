...
`commands: [...]` は「サブコマンドか、さもなくば素の positional か」の排他を表す糖衣。

- commands と元の positionals 全体が **排他**。
- **commands が先**、original_positionals が後 (完全経路として command 名にマッチすればそちら、しなければ素の positional として消費)。
- **commands 不在時は糖衣展開しない** (positionals はそのまま)。

これにより `git`（path を取らずサブコマンドだけ）も `git foo.txt`（command 名にマッチせず positional 消費）も一意に解ける。

具体的な lowering 形 (どの内部構造に落ちるか) は DR-041/042 の greedy 背骨に従う。当初想定した「positionals 内 or 挿入」式は Superseded (歴史) を参照。
...
当初「positionals 末尾に or 挿入」案が出たが、kawaz は「サブコマンドか、さもなくば素の positional か」の排他を or 一発で表現する展開イメージを示し、末尾挿入案は撤回された (具体的な展開式は Superseded を参照)。この排他の考え方自体は現役 (DR-041/042 の greedy 背骨で完全経路の一意性として実現される)。
...
- サブコマンドと素の positional の排他が完全経路の一意性で一意に解ける (正規形の具体構造は DR-041/042 の greedy 背骨。当初の「DR-004 の positionals 内 or に収束」整理は Superseded を参照)。
...
当初 `commands: [...]` は以下の式に展開されると定義していた:
...
- commands が先、original_positionals が後。
- commands 不在時は or で包まない (positionals はそのまま)。

kawaz の当時の展開イメージ (末尾挿入案を撤回した根拠):

> commands はこう展開されるイメージ
> positionals:[{type:or, children:[ ...commands, original_positionals ]}]

また効果として「正規形が前回 DR-004 の positionals 内 or に収束 = 過去 DR と合流」と整理していた。この or 収束による合流整理も、正規形が greedy 背骨へ移った DR-041/042 で失効している。
