...
**nameless seq 産の位置セル (DR-121 §3.2 の「3 座 seq セル」等) はこの表の住人ではない。**
2×2 は **value_type — 型として宣言できる形 — の分類**であり、seq セルは消費構造 (nameless
透過子の並び) が結果値に畳まれてできる**別平面の位置座**である。type の `out` に書けないので
union の variant にもなれない (DR-138 §1)。パス DSL の `[int]` がどちらにも届くのは、
セル空間 segment (seq セル、DR-127 §1) と値空間 segment (tuple、§4) が同じ表記を使うだけで、
空間が違う (DR-127 §2 の 2 相分解のまま)。null 残りの責務分担もこの表から読める — closed 側は「座が在る」ことを
