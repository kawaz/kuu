...
- **特殊値 inf**: `float` 型のみ受理。`number` 型では Error。受理語彙は慣用形 `inf` / `infinity` / `Inf` 等を **case-insensitive** で kuu が固定する。言語 DX (特定言語向け canonical types) が float の inf 非対応を選ぶことも許容 — その DX では未サポートで、必要ならユーザが registry に自作登録で解決する。
