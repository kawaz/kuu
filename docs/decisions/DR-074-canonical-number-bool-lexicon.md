...
- **`int` 型は値空間で判定** (10 進と同一規則): `0x1.8p3` = 12 は整数値なので受理、`0x1.8p0` = 1.5 は整数でないため int_round に従う (default `error` で `not_an_integer`) — 10 進の `1.5` が int で int_round に従うのと同じ規則を hex にも適用する。
  - **確定 (DR-075)**: この「10 進 int への一般化」(= 10 進 int も値空間で判定し、整数値の非整数構文 `3.0` を受理する読み = M2) は **値空間判定で確定** (DR-075)。int は整数「値」を受理し (`"3.0"`→3 / `"1e3"`→1000)、真に fractional な値 (`"2.5"` 等) のみが `kuu_int_parser` の config キー `int_round` (default `error`) に従う。DR-050 §4 の「JSON string は整数構文のみ parse 受理」(構文判定寄りの読み) は DR-075 §1 が supersede する。丸めモード (非 error) は値空間判定を論理的に強制する (DR-075 の M2 表裏関係)。
...
canonical = factory の default config。標準層 opt-in・方言は config キーの列挙 (DR-040 / DR-061)。`kuu_number_parser` / `kuu_bool_parser` / `kuu_int_parser` factory の config キー:
...
| `kuu_int_parser` | `int_round` | `"error"` | `"floor"` / `"half_even"` 等 | 非整数値の丸めモード 10 種 (DR-075)。int は値空間判定で整数値を受理、fractional のみ本キーに従う |
...
- DR-075 (int の値空間判定 + int_round — §2 暫定注記の M2 を値空間判定で確定、§4 config 表に `kuu_int_parser` / `int_round` を追加)
...
### `kuu_bool_parser` config キーの命名 (DR-100 で更新)

> **更新: DR-100 により `kuu_bool_parser` (DR-094 §9 案 A で `builtin/bool_parser` へリネーム済み) の config キー 3 つが `true_values`/`false_values`/`case_insensitive` から `bool_true_values`/`bool_false_values`/`bool_case_insensitive` へ改名された (`number_parser`/`int_parser` の factory 名 prefix 規約に追随、canonical は prefix ありと規約化)。§3/§4 の default 値・意味論は無傷 (機能不変のリネーム)。**
