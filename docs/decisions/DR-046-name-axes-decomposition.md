...
スコープ生成 (DR-025) と結果露出は**結果キー軸の責務のまま**。

> **更新 (TRG-Q4=a、kawaz 裁定 2026-08-15): 本節の「結果キー軸の責務」が正本に昇格した。** スコープ生成の判定は解決後の `export_key` 軸の占有で行い、name の presence では行わない (DR-025 / DR-033 / DR-006 を同裁定で改稿)。結果スコープと lexical スコープは同じ判定を共有する。id は参照解決のためだけに存在し、解決は lexical スコープ連鎖 (DR-032 / DR-033) で name と同じ空間を使う。id 未指定なら name が id を兼ねる (後方互換)。
