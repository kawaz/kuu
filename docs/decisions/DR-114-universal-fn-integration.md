...
variant DSL の effect 5 種を次のように読む。

| wire form | lowering | 意味 |
|---|---|---|
| `":set"` | 値スロット準備 | 発火時ユーザ引数を受ける入口。fn 呼び出しそのものではない |
| `"no:set:false"` / `"red:set:rgb:255:0:0"` | `cell_fns` の `set`、args は後続部品 | 引数値を `Value` として返し cell へ set |
| `":default"` | `cell_fns` の `default` | `use_default` sentinel を返し、default placeholder へ戻す |
| `":unset"` | `cell_fns` の `unset` | unset sentinel を返す |
| `":empty"` | `cell_fns` の `empty` | array / map を空にする sentinel を返す |
| `":update:increment"` | `cell_fns` の `update`、args の先頭は transform 名 | `EffectCtx` の現在値へ `filters` registry の transform を適用し、結果を cell へ set |

現行の `prefix:effect:arg...` 記法は維持する。これは `prefix:fn:arg...` の universal fn 糖衣であり、2 個目の部品が fn 名、3 個目以降が args である。
...
`update` は DR-077 の既存意味論を維持する。`EffectCtx` から現在値 `old` を取得し、args の先頭で指定した `filters` registry の **`output_mode: "transform"` かつ `fallibility: "total"`** の住人を残りの args とともに呼ぶ。未登録 transform は `unknown-vocab`、preserve または reject しうる filter と呼び出し席の不適合は `invalid-range`、args 不正は `invalid-argument` とする。
...
- `EffectCtx` は対象 cell、trigger、現在値 `old` を提供する
...
- `Value` を返し effect mode で現在値を変換する: `update` (`filters` registry の transform へ委譲)
...
3. 統一 `FnCtx` + mode 判別 + concrete `observes` 面
...
- `schema/descriptor.schema.json`: `role: "fn"`、`cell_fns` envelope、`observes` の role 条件分岐、`Value | Sentinel` 出力を反映
- `schema/builtin-descriptors.json`: `cell_fns` の builtin descriptor と filter の `observes` を反映
...
- `docs/LOWERING.md`: 3 DSL の common parse と specialization 別 lowering を追加
...
- lowering 期待値を universal fn 呼び出し形へ更新
- string / array 混在、colon を含む args、long からの env / borrow、default / effect の同一 fn、update + count 正規形、registry 分離、unknown-vocab / invalid-argument / invalid-range / circular-ref を追加
...
- `update` から fallible transform (`output_mode: "transform"` + `fallibility: "reject"`) を呼ぶ意味論。v1 は total transform に限定する
...
- DR-045 / DR-077 (cell operation descriptor と update effect)
