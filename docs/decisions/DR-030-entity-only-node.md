...
> **更新 (DR-140 §2、2026-08-16): `value:` の綴りは値カプセル内の `const` field へ移った**
> (`{"name":"timeout","value":{"const":30}}`)。実体だけノードの意味論 (消費 0・初期化位相・
> source `const` — DR-031) は不変で、判別は 「**`value` が不在、または `value` の type が `"none"` である**」という判別式で行う (`type: "none"` はカプセル語彙上「カプセル ⊘」だが wire 綴りとしては `"value": "none"` の縮退形を持つため、presence 単独では取れない)。
>
> **訂正 (2026-08-16): 「presence で従来同様に成立する」としていたのは不正確。** DR-140 §4 は「`value` の presence 単独では取れない」と明示しており、上の 2 条件形が正確な判別式である。
...
実体だけノードは基本 result に出す (それが目的)。ただし link 専用の中間セル (内部計算用の一時セル) は result に出したくない場合がある。`export_key: null` で「値は持つが result に出さない、link 元としてだけ存在」を表現 (DR-052)。
