...
> (`value` 属性、DR-140 §1) の presence で構造的に判定される形になった。** 分界の意味論は不変。
...
**露出キーを持ち値セルを持つ通常要素**であり、結果アドレス軸 (`result` / `sources`) にも宣言名軸
...
- `effects[].entity` は**宣言名軸**であり `export_key` を適用しない (DR-121 §5) ので、
...
- **宣言名軸の一意性** — 同一スコープの宣言名重複 (config_file `user` + option `user`) は露出キー軸とは
  別軸の問題であり、報告に使う kind は未確定 (`docs/QUESTIONS.md` DNR-Q1)。本 DR は露出キー軸で
  掛かる範囲だけを確定させる
