...
- **pattern の compile 失敗は definition-error** (kind=invalid-argument、波及参照)。pattern は定義の一部で compile 可否は定義時に静的に判定できる (DR-083 §5 の「静的に既知は定義時に倒す」と同筋、kawaz 裁定 2026-07-10)。照合不一致 (実行時の reject) とは層が異なる。方言 (§2) の帰結として compile 可否は host 依存になる — ある host で valid な定義が別 host で definition-error になり得るが、compile できない pattern はその host では動作し得ないので、全入力を silent reject するより定義時 fail-fast が正しい
...
- DR-054/082 の definition-error kind 語彙に **invalid-argument** (registry 装置に渡した引数値そのものの不正) を追加。fixtures/definition-error/ に regex compile 失敗の fixture (概ね全方言で不正な `[` 級の pattern に限る — 方言安全性の裏返し)
- fixtures: fixtures/piece-filters/ に regex_match の輪郭を追加 (一致受理 / 不一致 reject / 部分一致と全体一致の区別 / メタ文字 escape。いずれも方言安全な共通パターンのみ。colon 含み pattern のオブジェクト形式 case を含める)
