...
> **更新 (2026-08-16、3 点):**
>
> 1. **「wire に id は露出しない」は現役ではない** — `schema/wire.schema.json` の `$defs.node` は
>    第一級の `id` property を持つ (DR-046 §2 で nameless 要素への ref/link を可能にするために
>    導入)。露出しないのは **`#` を含む内部 id** (DR-046 §4 / DR-063 §3) だけである。
> 2. **参照キーの照合は id 軸の 2 段ルックアップ** (DR-136 §6) — 各スコープで raw 一致 → 写像後
>    一致の順に照合し、無ければ外側へ。「参照キーは name」は「name が id 軸へ供給した値で照合
>    される」と読む。`definitions` のキーは書き手の literal なので段 1 のみ。
> 3. **alias は id 軸を占有しない** (DR-054 更新 5 / DR-120 §4) — 参照ファミリーのうち
>    `ref` / `link` は自前の宣言として id を持つが、alias 入口は canonical を指すだけである。
