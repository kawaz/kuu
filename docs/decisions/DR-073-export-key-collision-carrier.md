...
> **追記 (UX-Q7R、kawaz 再裁定 2026-07-16)**: 未発火実体の preset / 宣言 default は共露出に現れない — これは本 DR の衝突検査への例外ではなく、default 注入の充填判定が export_key 適用後の結果 cell 単位で行われる (正本は DR-031 追記 note) ことの帰結。共露出キーが他実体の値で埋まっていれば default は注入されず、「実際の共露出」(DESIGN §15.5) が成立しないため衝突検査の入力に最初から乗らない。逆に **default より上の席 (env / config / inherit) の充填は実際の共露出に数える** — 異なる実体の上位席同士 (例: a の cli と b の env) はラダー順位 (DR-031) で競合解決されない。ラダーは 1 実体の値 cell 内の規則であり、実体間のキー占有は provenance 競合 = ambiguous (§2 のとおり値でも source でも区別不能で、claimants が解釈を区別する)。
