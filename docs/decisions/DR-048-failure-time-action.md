...
**候補経路は dead end を含む**: 「候補経路で selected」の候補には、途中で失敗した partial 経路 (dead end) を含める — scope の末尾まで到達した経路に絞らない。`mytool --help abc` (abc が後続の number positional で型失敗) でも help が発火する。slice PoC 第 6 弾の 3 候補定義 (scope-end Accept のみ / dead end 込み / 最深消費のみ) の観測マトリクスで確定した: scope-end 限定は失敗形によって発火が不安定になり、最深消費限定は先食い (「help を撃たずに深く進む候補」が存在しない) により dead end 込みと実質一致する。partial 経路の保持は F-039 (失敗時部分状態) と接続する。

**ambiguous では発火しない**: 発火条件は完全経路 0 本のみで、2+ 本 (ambiguous) では発火しない。ambiguous の本体は競合解釈の提示であり、大きな help がエラー内容を画面から流すのは UX を損なう。0 本失敗 (使い方が分からない → help が有益) と ambiguous (どの解釈か明確化してほしい → エラーが本体) では見せたい情報が違う。

**--help への誘導行 (方向)**: help 入口が定義されている場合、0 本 / ambiguous 両方のエラー表示末尾に `--help` へ誘導する 1 行を出す (GNU 慣習 `Try 'prog --help' for more information.` と同型)。表示の大きさで発火範囲が変わる 2 段構え — help 本体 (大) は 0 本のみ、誘導行 (1 行) は 0 本 + ambiguous。誘導行の綴りは定義の help 入口から自動導出できる。具体仕様 (文言・条件・JSON 構造上の表現) は本 DR では確定しない (F-043 / help installer の設計の関心)。
...
「候補経路で selected」の精密定義 (§2 の dead end 込み) は、issue `2026-07-03-failure-action-selected-scope` でトラッキングし、slice PoC 第 6 弾の観測マトリクスを経て確定した (当時の journal はリポ再編で本リポに存在しない)。
...
### 候補経路を scope-end 到達経路に限定する (AcceptsOnly)

`--help abc` (後続 positional の型失敗で dead end) を見落とし、失敗の形によって発火が不安定になる。slice PoC 第 6 弾で観測し不採用。

### ambiguous でも help 本体を発火する (on: not-success)

ambiguous の本体であるエラー内容 (競合解釈の提示) が大きな help で画面から流れ、UX を損なう。help を求める人のカバーは誘導行 (§2) が担う。
