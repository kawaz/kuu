# DR-107: descriptor の直交軸化 — role / construction / io_type / output_mode / fallibility / invocation
...
### 4. `signature` を `output_mode` + `fallibility` に分解 (DAX-Q4=a)

```
output_mode: "preserve" | "transform"        // 入力を変えずに検証するか、新しい値を計算するか
...
フィールド名は当初 `effect` として起草したが、kuu では `effect`/`effects` が既に cell operation descriptor の中核語彙として確立している (DESIGN §7.4 の「effect 語彙 (4種)」表、DR-045 の効果記述子 `{op, operand, ...}`、CONFORMANCE.md §2 の `effects[].op` 7-op表) — この既存語彙と字面上・意味論上いずれも無関係な軸に同じ名前を再利用すると紛らわしい (codex レビュー #4 A-M6 の指摘、VE-Q1 裁定 kawaz 2026-07-15=a)。よって `output_mode` に rename する (値 `preserve`/`transform` は不変)。
...
`output_mode:"preserve"` は入力値をそのまま返す (成功時に同じ値を返す) ことを意味し、`io_type.input == io_type.output` を伴う — この不変量は §7 の role 別マトリクスで Schema の if/then により強制する (codex レビュー #4 A-M6(2)/B-C4 の反映、JSON Schema の再帰型比較の限界により厳密な型等価の強制はできないため、`io_type.input`/`io_type.output` の value_type 表現が構造的に一致することを検査する semantic lint の対象と位置づける)。

`role:"collector"` は本 DR §7 の role 条件分岐により `output_mode:"transform"` + `fallibility:"total"` に固定される (DR-105 §4「filter 席は fallible 優勢、構造畳み装置 (accumulator/collector) は total」の勢力図の descriptor 上での機械的強制)。
...
> **追記 (DR-111、2026-07-17): 上記 `accumulator` / `completer` 行の「未確定のまま非制約」は解消された。** accumulator は collector と同じ「構造畳み装置」の勢力図で確定 (io_type 必須・input 常に array ラップ・output_mode=transform/fallibility=total/construction=static の const 固定・invocation は object_args 固定・domain/owns/observes/config 禁止)、completer は named capability marker の最小形で確定 (invocation none 固定・construction static 固定・reasons 常時空・io_type は runtime 問い合わせ ABI 確定まで禁止)。確定行の全文・導出根拠は DR-111 §1/§5 のマトリクスが正本。builtin accumulator 3 種 (append/merge/kv_map) の収載も DR-111 §3。

`domain:"array"` (collector) と `output_mode:"transform"`/`fallibility:"total"` (collector) を const 固定するのは、これらが DR-036/DR-044 (collector の入力は常に累積後配列) と DR-105 §4 (構造畳み装置は total) から構造的に導かれる不変量であり、descriptor 単位で書き分ける余地がないため。`provider` の `output_mode`/`fallibility` 禁止は、`null` 返却が filter の reject/reason 機構と異なる意味論 (§6) であることの Schema 上の反映。
...
- DR-095 の reasons 宣言集合の正本性 (spec 側が正本、実装は宣言集合の部分集合を emit) は不変 — `role`/`domain`/`io_type`/`output_mode`/`fallibility`/`invocation` いずれも「実装挙動を変えない宣言」という DR-061 §4「descriptor は validator ではない」の位置づけを継承する
...
- **schema/descriptor.schema.json**: 全面改訂 (`kind`→`role`、`construction`/`io_type`/`output_mode`/`fallibility`/`invocation` 新設、role 別条件分岐拡張)
- **schema/builtin-descriptors.json**: 既存 13 住人 (filter 7 + collector 2 + type_parser 4) を新軸へ migration、provider 3 種を新規収載 (計 16 住人)
- **docs/REFERENCE.md**: §6 builtin filter カタログの `kind`/`domain`/`signature` 列を `role`/`domain`/`output_mode`/`fallibility` に更新 (filter 名一覧そのものは不変、`just lint-reference` はフィールド名を直接検査しないため機械検査への影響なし)
