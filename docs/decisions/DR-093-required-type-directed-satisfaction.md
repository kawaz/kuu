...
`{"type": "dd", "required": true}` (canonical では `{"name": "--", "type": "dd", "required": true}` の一部、DR-064 §5 の name デフォルト供給と合成) で kawaz/die の `--` 必須が表現できる。`die msg` (`--` 未発火) は制約 `required_violated` の failure になる。
