...
> **更新 (DR-130 §が名指しした null 読み替え、2026-08-16): 「値が在る」の判定に `null` を含めない。**
> DR-130 で absent の表現が `null` に統一されたため、**未供給の座は `null` として result に現れる**。
> したがって required の充足判定は「キーが在るか」ではなく「**値が `null` でないか**」で読む
> (bool 目的語の requires が「解決後の値が true」で判定される DR-047 §5 と同じ流儀 — 型ごとに
> 「在る」の意味が決まる)。
