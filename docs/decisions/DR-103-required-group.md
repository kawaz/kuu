...
```json
{"name": "create",  "exclusive_group": ["mode"], "required_group": ["mode"]}
{"name": "extract",  "exclusive_group": ["mode"], "required_group": ["mode"]}
{"name": "list",     "exclusive_group": ["mode"], "required_group": ["mode"]}
```
...
- **fixtures/constraints-parse/**: `required_group` 系新設。既存 `exclusive.json`/`required-*.json`/`default-interaction.json` の輪郭に倣い、以下の軸を pin する: (a) 充足 (1 member 発火で success)、(b) 違反 (0 member 発火で `required_group_violated`)、(c) default との相互作用 (member の 1 つが default を持つ場合、default による値充足で自動的にグループ充足 — `required` の「required + default は常に充足」DR-047 §6 と同型の帰結)、(d) `exclusive_group` 同名併用による exactly-one (tar 型の直接表現)、(e) 縮退 (単独 member)
