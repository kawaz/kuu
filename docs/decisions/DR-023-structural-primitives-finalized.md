...
> **更新: 本 DR の以下の記述は後続 DR で覆された。現役仕様の理解には不要、判断経緯としてのみ残す。本 DR の「4プリミティブ + 糖衣展開」という骨格自体は引き続き有効。**
>
> **訂正 (2026-08-16): 上記が「4プリミティブ + `multiple` 属性 + 糖衣展開」としていたのは現役では
> ない。** `multiple` 属性は DR-139 §5 で廃止され (cardinality は値カプセルの座が担う)、骨格に
> 残るのは 4 プリミティブと糖衣展開である。
