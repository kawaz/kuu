...
本表が語るのは**露出キー軸だけ**である。参照識別子 (id) 軸の重複は DR-054 更新 5 の `duplicate-id` が別途判定するので、4 行目の同名ペアは露出キー軸では合法のまま (`id` 未指定で name が id 軸へ供給されるため) `duplicate-id` に掛かり、1 行目の同名ペアは 2 軸ともに破れて要素ごとに 2 kind が立つ。
...
- `link` を持つ参照ノード / `alias` 要素 — 値は canonical の実体セルへ流れ、自前のセルを持たない
  (DR-029、DR-057「結果キーは canonical のみ」)。name は綴り軸にのみ効く — **alias 入口は id 軸も
  占有しない** ので `duplicate-id` にも参加しない (DR-054 更新 5)。`link` を持つ参照ノードは自前の
  宣言なので id 軸は普通に持ち、露出キー軸にだけ参加しない
...
- **綴り (トリガ literal) 軸は本 DR の対象外** — 同一スコープのトリガ重複は静的 warn + 実行時
  ambiguous のまま (DR-041 / DR-059 §2 / DR-067 §2)。同名 command 2 本は綴り軸では合法で、結果キー軸で
  衝突する (command の name はトリガ綴り軸へ供給する、DR-052 §2 / DR-136 §1〜§3 — 綴りそのものは
  `trigger_name` 軸が持ち、name は写像を通してそこへ供給する)。ただし `export_key` を割っても解けるのは
...
- DR-041 / DR-067 §2 (トリガ綴りの重複は warn + 実行時 ambiguous — 軸の分離、§7)
