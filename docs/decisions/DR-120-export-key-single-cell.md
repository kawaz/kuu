...
本表が語るのは**露出キー軸だけ**である。宣言名軸 (参照識別子の重複) は DR-054 更新 5 の `duplicate-name` が別途判定するので、4 行目の同名ペアは露出キー軸では合法のまま `duplicate-name` に掛かり、1 行目の同名ペアは 2 軸ともに破れて要素ごとに 2 kind が立つ。
