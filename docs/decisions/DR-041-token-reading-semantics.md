...
`--key=value` の分解や連結 short (`-abc`) の展開のような **トークン境界の再解釈**を、評価器の実行時意味論として以下の 5 則で定義する。再解釈の実行時能力 (matcher) は installer (DR-042) が parse_definition 時に植え付けるものであり、AtomicAST に専用のノード型・フィールドは追加しない。
...
matcher は 1 トークンに対して複数の読みを**枝として全列挙**し、絞り込まない。DR-038 の「入力を全消費する完全経路がちょうど1本」は、**読みの選択を含む全探索空間**に適用される。
...
### 4. 先食い: greedy はスコープの positional 面に優先する

先食いは**スコープの positional 面の消費点**で働く。次トークンがそのスコープの**トリガ語彙** — (a) greedy エントリが読めるトークン、(b) スコープで宣言され未発火の dd マーカー (`--` 等) — に一致する場合、素通し消費 (positional がそのトークンを raw で食う読み) は枝として生成しない。素通しが許されるのはトリガ不一致 (読みゼロ) の時のみ。

(b) を欠くと、dd を宣言した定義でも寛容な string positional が `--` そのものを値として食う枝が完結し、`prog -a -- -b` が常に ambiguous になる (垂直スライス PoC で実測)。dd マーカーは matcher を持たない純構造のまま、抑制トリガ集合にだけ載る。

- **positional 面は多孔質**: スコープ直下の positional 列は seq の連接として平坦化された 1 本のトークン列として消費され、その各消費点の間に greedy が割り込める。`cp a --verbose b` は a → `--verbose` 発火 → b と消費される。multiple の反復間も同様 (`cp src... --verbose dst` が成立する根拠)。
- **greedy の内部消費は一体 (bound)**: greedy が発火したら、その内部構造 (値スロットの seq 等) は後続トークンを**無条件に raw 消費**する。`--color -v` は `-v` が定義済みでも `color="-v"` になる (getopt 同型)。トリガトークンが引数の束縛を宣言する、というのが greedy の意味論であり、外側スコープの greedy 面は発火中の greedy の内部消費に割り込まない。
- `prog -v src` (`-v` 定義済み、src は string positional) → scope 面の `-v` は greedy 発火のみで `src="-v"` の読みは立たない。リテラル `-v` を **scope の positional** に渡すには dd (`--`) を使う (greedy の値スロットへは raw 消費されるため escape 不要)。
...
### dashdash (`--`) — matcher 不要の純構造

dd は UsefulAST の糖衣であり (DR-014 の「config に含めず AST に直接書く」判断を踏襲)、dd installer (DR-042) が**再スコープ化**として展開する: 継続 (以降の positional 列) を greedy 面を断った子スコープに包む。

「スコープ境界は greedy 面を断つ」(DR-042 不変則⑤) により、`--` 以降では matcher の規則が構成されず、全トークンが素通しで positional に落ちる (発火後のスコープに dd トリガは残らないため、2 個目の `--` は raw 消費できる)。

AtomicAST に専用ノード型・状態フィールドは不要。sever (断ち) は**探索経路局所の状態**であり、消費位置と同種 — 経路と独立した大域 mode ではない。実装は「継続を子スコープに包む構造」でも「経路状態の sever フラグ + 純マーカー」でも、観測挙動が同一であればよい (PoC は後者を採った)。**dd はトークン境界を再解釈しないため matcher を持たない** (`--` 自体は素の exact 一致で、§4 トリガ語彙の (b) に載る)。
