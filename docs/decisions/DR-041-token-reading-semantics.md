...
先食いは**スコープの positional 面の消費点**で働く。次トークンがそのスコープの greedy エントリに読める場合、素通し消費 (positional がそのトークンを raw で食う読み) は枝として生成しない。素通しが許されるのは greedy 不一致 (読みゼロ) の時のみ。
...
「スコープ境界は greedy 面を断つ」(DR-042 不変則⑤) により、`--` 以降では matcher の規則が構成されず、全トークンが素通しで positional に落ちる。専用ノード型・mode フラグ・評価器状態は一切不要。**dd はトークン境界を再解釈しないため matcher を持たない** (`--` 自体は素の exact 一致)。
