...
読みは**外向き消費数** (背骨・path-search の世界の通貨) と効果 (DR-045) を報告する。消費数は Accept の報告値であり、**value の有無から導出しない** (消費 0 の literal 産出も、消費 2 の外側借用もある)。再解釈 matcher の内部断片消費は matcher 内に閉じる — 外向き 1 トークンを内部で複数断片に割る方向 (eq-split の `--k=v`) と、外側の次トークンを借りて外向き 2 になる方向 (short 値付着) の両方がある。
...
- **背骨の範囲**: command 部分木は新しい背骨を宣言し、祖先の greedy はそこに届かない (スコープ越えは DR-042 の global installer による構造コピーで表現)。発火した greedy の内部消費 (値スロット、dd の継続) には背骨がなく、何も発火しない。それ以外の構造 (ref 再帰の unfold 等) は現在の背骨に留まる。**command 部分木の完了後は親の背骨が再開する** (親の greedy は末尾でも有効)。発火した command が親を恒久的に断つ規則 (sever) は持たない — 必要な一意化は次項の早閉じ抑制が担う。
- **早閉じ抑制**: 背骨を持つスコープは、**自分の背骨で読めるトークンが次に控えている間は完了できない**。先食いの適用先を「素通し消費」から「スコープ退出」にも広げた同型規則 (その場で読めるものは、その場が読む)。これにより「読める最内スコープが勝つ」が経路間の優先なしに創発する: app と build が別実体の `-o` を持つとき `app build -o X` は build 内で `-o` が読める間 build を閉じられないため build 側が一意に食い、`app -o X build` なら root 側が食う。末尾の親オプション (`app build x --rootopt`、build に無いトリガ) は build が閉じられるので親が食える。
...
- **同一トリガの解決 (shadowing)**: 複数スコープ由来のトリガが同じ消費点で衝突する場合 (global コピー等) は最小スコープの宣言が食う — lexical 解決 (DR-032/033) のパース時適用。判定単位は**要素名ではなくトリガ literal** (別名の要素でも同じ `--verbose` を持てば衝突する。垂直スライス PoC で name 判定の不足を実測)。同一スコープ内の重複は従来どおり静的 warn + 実行時 ambiguous (DR-021/038)。
...
スコープで回収された short エントリ + config (`short_combine` / `short_prefix`) から規則を構成。cluster 分解・値付着の全解釈を読みとして列挙する。曖昧になる定義 (`-cv` 問題) は正しく ambiguous になる。分割単位: **canonical の short は ASCII 単一文字**とし、Unicode (grapheme) short は方言として扱う。他 CLI 互換 (「short の値付着は末尾のみ」等) も installer のパラメータとして狭める方言で吸収し、コアの責務にしない。
...
### §4 先食い・早閉じ抑制の述語「読める」 (DR-097 で更新)

> **更新: DR-097 により §4 の述語「読める」は『トリガ一致』ではなく『その greedy 読みを経由する完全経路が存在すること (読みの成立)』に精密化された。トリガが一致しても読みが完全経路に届かない消費点は読みゼロと同じ扱いになり、素通し枝が生きる。§4 本文の「先食い: 背骨上の消費点で、そこに到達可能な greedy エントリが読めるトークンは、素通し消費の枝を生成しない。素通しはトリガ不一致 (読みゼロ) の時のみ」にある「読める」「トリガ不一致 (読みゼロ)」は、この精密化された定義で読み替える。早閉じ抑制 (同型規則) にも同じ述語が及ぶため同様に更新される。**
