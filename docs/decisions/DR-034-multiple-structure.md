...
各 piece に対して piece_processor を適用:
  piece: String
...
- accumulator: override (prevs を無視して `[processor(piece)]` を返す)
- collector: unwrap_single (`[t] → t`、要素数1の unwrap)

結果として「multiple 無し = piece_processor 一本で終わる」と同等になる。
...
type が主に指す断面:    processor, default, 入力消費数 (accumulator/collector はデフォルト)
multiple が主に指す断面: accumulator, collector, separator (processor/default はデフォルト)
```

両者が同じ属性 (例: accumulator) を指す場合、合成順:
...
→ multiple の責務を「複数値経路 (accumulator/collector/separator) を起動するスイッチ」に絞る。個数制約 (positional のリピート) は別軸として分離。
...
→ accumulator のシグネチャを `(piece: String, processor, prevs: T[]) → T[]` にすることで、mergeable も append も override も同じ枠で表現可能。kuu core の `Accumulator[T, U] = (T, U) → T` を一般化したもの。
...
- [external: kuu.mbt DR-023] (mergeable リスト、本 DR の accumulator シグネチャでサポート可能に)
...
> **更新: DR-036 により本 DR の collector 独立 registry 構想は廃案。現役では filters registry に統合。本 DR の piece_processor/separator/accumulator/collector の4要素構造自体は引き続き有効。**
...
> **更新: DR-036 により accumulators registry エントリは「属性セット」(accumulator + default_collector + default_separator) に拡張され、multiple registry が新設された。本 DR の accumulator シグネチャ自体は引き続き有効。**

本 DR では accumulator を単純関数として扱っていたが、DR-036 で **accumulators registry エントリを「属性セット」(accumulator + default_collector + default_separator) に拡張** + **multiple registry を新設** することで、multiple プリセットを参照糖衣として一貫させた。
