...
> **更新 (2026-08-16、2 点): requires の意味論が二重に精密化されている。**
>
> - **目的語の充足判定は型委譲** — 値空間を持つ要素は「最終状態の値の有無」、bool 目的語は
>   「解決後の値が true」で判定する (DR-047 §5 / DR-093 §1)
> - **参照の解決は id 軸の 2 段ルックアップ** (DR-136 §6) — raw 一致 → 写像後一致 の順。
>   本 DR の例が `"requires": ["key-file"]` のように name の綴りで書いていても段 2 で届く
