...
### `auto_env` の命名 (DR-100 で更新)

> **更新: DR-100 により config キー名が `auto_env` から `env_auto` へ改名された (DR-096 の軸 prefix 先頭規律を env 軸にも適用、`env_prefix` と並びが揃う)。§3 の名前導出規則・意味論は無傷 (機能不変のリネーム)。**
