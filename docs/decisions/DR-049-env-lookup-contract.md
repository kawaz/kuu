...
> 由来: findings `2026-06-29-ast-missing-pieces.md` の F-030 (auto_env の名前変換規則) / F-031 (env_provider のインターフェース契約)。LOWERING G-10 に対応する。本セッションで確定 (深い分岐が無いことを確認の上、設計案をそのまま採用)。
...
env から来た値は string であり、要素の peaceProcessor (pre_filters → parse → post_filters、DR-034) を通る。multiple 要素なら separator 分割も効く (発火 1 回の 1 引数と同じ扱い)。variant の args が「全て string で CLI 引数パースと同じ手順を通る」(DR-011) のと同じ流儀であり、env だけの特別な型経路を作らない。
...
POSIX では空の環境変数は存在する状態。値の有効性判定は provider の関心ではなく filter (peaceProcessor) の関心 (non_empty を書けばよい)。
...
- config ファイル値源 (config_provider / `type: "config_file"` / オブジェクトパス対応) は F-029 として別 DR で確定する
...
- DR-034 (peaceProcessor — env 値の通過経路)
...
- findings `2026-06-29-ast-missing-pieces.md` F-030 / F-031 (解消)、F-029 (射程外)、F-033 (射程外)
