...
> **更新 (DR-139 §1.2、CR-Q1=a 裁定 2026-08-16): §2 の命名 (作用対象アンカー) は部分
> superseded — §採用しなかった案「アンカー全明示」が再採用された。** カプセル field 名は
> `pre_type_filters` / `post_type_filters` / `final_filters` (綴り不変) /
> `post_accum_filters`。棄却理由 (冗長で wire に重い) は要素直下フラット前提の判断で、
> カプセル入れ子化により無効化された。全てアンカー (type / accum) を明示するため、本 DR §2
> が排除した「別軸が対に見える」非対称は再来しない。§1 の座席格子 (A/B/C/D1/D2 の構造)・
> §3 の `raw_filters` 予約 (配線時は本体系のアンカー明示へ揃える)・§4 の registry 純化・
> §5 は生存。
...
> **配置の訂正 (2026-08-16)**: 本節は当初 `## Superseded (歴史)` 配下に置かれていたが、
> D→D1/D2 分割は**現役の構造**であり (本 DR ヘッダの注記も「生存」と宣言している)、
> 歴史記録ではない。節を `## 現役の更新` へ移した。
