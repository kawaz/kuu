...
> **追補 (2026-08-16): 座席名は 2 世代進んでいる。** DR-034 の再構成 → DR-079/DR-102 の 4 座席
> (`piece_filters` / `value_filters` / `final_filters` / `accum_filters`) → DR-139 §1.2 の
> カプセル内 5 座席、という変遷であり、本 DR の段名・`multiple.kind` の綴りはいずれも現役ではない。
> 段を分けるという判断そのものは生きている。
...
### `@base` sentinel は二形表記に置換

> **更新: DR-062 により `@base` sentinel は廃止。継承合成は filters フィールドの string 短縮形 (配列 = 差し替え) | object 詳細形 (`{prepend, append}`) で表す。本 DR の継承元解決順 (ref 元 → type registry デフォルト → 空) は DR-062 でも不変。**

廃止理由は in-band 特殊値の発見不能性と sentinel パターンの孤立 (仕様中 1 箇所のみ)。詳細は DR-062 を参照。
