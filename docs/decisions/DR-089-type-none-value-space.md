...
> **更新 (DR-130、2026-08-01): `type: "none"` は値空間を持たないため露出キーを持たず、成功 result の宣言キー全列挙対象にもならない。** null は値空間の住人になったが、値空間そのものが無い node に null 座を新設しない。消費との直交は不変。
...
> **一部 Superseded by DR-093**: 「値充足を要求される席には立てない → definition-error」は required / requires の目的語について置換される。required / requires の充足判定を型 (値空間) への委譲として一様定式化すると、none の充足定義は「発火した (committed)」であり、definition-error にする理由がない。本節冒頭の committed 基準制約 (conflicts / exclusive / requires トリガ側) への参加は不変。
