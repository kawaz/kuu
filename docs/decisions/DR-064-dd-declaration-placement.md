...
> **更新 (2026-08-16、3 点):**
>
> 1. **「トリガ綴り軸にのみ効く」は結果キー軸との対比であって、id 軸への供給を否定しない** —
>    dd の name も id 軸へは文字写像を通して供給される (`--` の dd の参照識別子は `__`、
>    DR-136 §2)。参照側は 2 段ルックアップで `requires: "--"` からでも届く (DR-136 §6)
> 2. **dd のトリガ綴りだけは name の literal 直値** (写像を通さない、DR-136 §2) — 方言指定
>    `{"name":"++"}` が写像で `--` に化けないための特例
> 3. **`{}` 規定の出所は DR-051 ではなく DR-052 §3 → 現 DR-130** (DR-051 は全体 Superseded)。
>    あわせて absent の表現は DR-130 で `null` に統一された
