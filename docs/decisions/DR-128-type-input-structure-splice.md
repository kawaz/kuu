...
> **更新 (DR-137 §1、2026-08-16): この「構文衝突を抱える」という根拠は無効になった。** tuple は
> タグ付き形 `{"tuple": [...]}` で導入されたので bare array union とは衝突しない (本 DR 内の別節が
> 既にそう記している)。本節が不採用の理由に挙げた衝突は現存しないため、判断の根拠としては読まない。
...
> **Superseded by DR-137 (UC-Q1=a、2026-08-16)**: tuple 型は value_type 体系へ第一級で
> 追加された (タグ付き形 `{"tuple": [...]}` — bare array union との構文衝突は回避)。
> 本棄却の理由のうち**入力側** (「pair は定義片の `seq` がそのまま表す」) は不変であり、
> input_structure の役割は変わらない。覆ったのは前提の射程で、union 値の枝並行構築
> (DR-138) が要求する**出力側の位置付き部分構築の座**は seq では書けない。
