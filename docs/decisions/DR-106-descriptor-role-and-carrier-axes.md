...
> **Status: Superseded by DR-107 (部分)。** `kind` enum (役割軸の宣言形) と `signature` の複合軸限界は DR-107 (descriptor の直交軸化) が置き換えた — 詳細は本文末尾「Superseded (歴史)」節。**明確化 (a)(b)** (`domain` と registry 登録の一致義務、合法性判定は自 registry の owns 集合のみ) は registry lookup の挙動規定として **現役のまま新構造 (`role`/`domain`) に直接適用される** — 「Superseded」は `kind`/`domain` の宣言形にのみ及び、明確化 (a)(b) には及ばない (codex レビュー #4 B-Min4 の反映、旧 banner は「以下は全て歴史」と読める書き方で明確化 (a)(b) が現役適用される事実と矛盾していた)。
...
>
> **更新 (DR-139 §1.2、2026-08-16): 本バナーが「現役のまま適用」と宣言する (a)(b) の**判断**は
> 生きているが、列挙されている座席名は旧 4 座席である。DR-139 §1.2 で座席は値カプセル内の
> **5 座席**へ再編されるため、座席名は移送後の綴りに読み替える (移送の作業単位は
...
本節は DR-107 (descriptor の直交軸化) との関係を **superseded (覆された部分)** と **retained (現役のまま適用される部分)** に分けて明示する。両者を同一の「以下は歴史」枠に混在させていた旧記述 (codex レビュー #4 B-Min4 が指摘した内部矛盾) を解消するための構造分離であり、判断内容自体は変更しない。

### Superseded — `kind` enum と `signature` の宣言形

> **現役仕様の理解には不要、判断経緯としてのみ残す。**

`kind` フィールド (`installer`/`factory`/`filter`/`collector` の 4 値) は `role` に rename され、初期集合が 7 値 (installer/filter/collector/type_parser/accumulator/completer/provider) に拡張された (DR-107 §1)。`domain` (scalar|array) フィールド自体は継続するが適用範囲が `kind:"filter"` 限定から `role:"filter"`/`role:"collector"` の両方に広がり、collector では `domain:"array"` に const 固定される (collector の入力は常に累積後配列という DR-036/DR-044 の構造的不変量、DR-107 §7)。明確化 (c) が「軸の分離は issue で追跡する」と明記していた `signature` の複合軸限界 (変換×失敗可能の第 3 象限が表現不能) は、DR-107 §4 の `output_mode`/`fallibility` 分解で解消された (DR-107 起草時点のフィールド名は `effect` だったが、既存の cell operation `effect` 語彙との衝突を避けるため `output_mode` に rename、VE-Q1 裁定)。

### Retained — 明確化 (a)(b) は registry lookup の挙動規定として現役適用

> **以下は覆されていない。DR-107 の新構造 (`role`/`domain`) にそのまま適用される。**

明確化 (a) (`domain` と registry 登録の一致義務) と明確化 (b) (合法性判定は自 registry の owns 集合のみで決まる、descriptor の `kind`/`domain` メタデータは lookup 自体に影響しない) は、`kind`/`domain` という**フィールド名の宣言形**ではなく **registry lookup という挙動そのもの**を規定しており、DR-107 の `role`/`domain` へのフィールド rename・拡張とは独立に成立し続ける (DR-107 §2 も「DR-106 明確化 (a)(b) は本 DR の対象外…不変のまま role/domain の新構造にそのまま適用される」と明記)。
