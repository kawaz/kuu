...
- **`sources` (optional)**: 露出キー → 値源タグ (`cli` / `link` / `env` / `config` / `inherit` / `tty` / `default`、DR-098 §6 が DR-031 の 6 語彙に `tty` を足した集合が正本) のマップ。`link` は他要素の入口から link で飛んできた効果が最終値を確定させた場合 (自分の入口からなら `cli`、DR-031 の source 確定ルール) — ラダー同順位で経路の違いのみ。最終値の由来 (ParserContext の source メタ、DR-031) を検証する — 値源系 fixture で使用。effects が cli / link 効果のみである規約は不変 (ラダー充填の順序を effects に持ち込まず、由来の検証は本フィールドが担う)。**キーは `result` と同一のキー体系** — すなわち **export_key 適用後の露出キーを scope-path 修飾したもの** (root 直下は `"ttl"`、入れ子 scope 内のセルは `"sub.ttl"`)。scope path の各セグメントも射影後の露出キーであり、`export_key: null` の scope 生成要素はパスに現れない (子が昇格した先のパスで書く、`fixtures/export-key/command-promote.json`)。`export_key: null` の値セルは結果キー軸を持たないので `sources` にも現れない。

  `sources` が席を持つのは **値源ラダー (DR-031) で確定する値セル** — leaf と accumulator — に限る。したがって `result` のキー集合と `sources` のキー集合は次の 2 点でのみ食い違う:

  - **スコープの presence marker** (選ばれた command / name を持つ `or` / `seq` が持つ空 kv `{}`、DR-052 §3) は `sources` に席を持たない。スコープ生成要素は結果キーを占有する 1 セルとして数えるが (DR-120 §4)、その占有はスコープの存在自体であってラダーが確定する値ではないので、報告すべき値源が無い
  - **空の accumulator セル** (`result` に `[]` として現れる、DR-044) は逆に `sources` にも `default` として現れる。0 回発火でも結果キーと値 (`[]`) を持ち、その値をラダー最下段が確定させているため (DR-031 の default 席 × DR-044 の uniform array 既定の合成)。

  `sources` のキーは `effects[].entity` (射影前の値セル name、`fixtures/export-key/rename.json`) とは**別の名前空間**である点に注意 — `{"name":"verbose","export_key":"v"}` に `--verbose` を与えた場合、`effects[].entity` は `"verbose"`、`result` / `sources` のキーはともに `"v"` (`fixtures/export-key/rename-flag-default.json`)。射影前 name をキーにすると、同名別要素が相異なる export_key を持つケース (`fixtures/export-key/same-name-distinct-keys.json`) や透過要素と他要素の export_key が同綴りになるケース (`fixtures/export-key/non-participants-coexist.json`) で一意化が破れる。同名セルが複数 scope に存在するケース (inheritable の祖先 write-target 等) の一意化は scope-path 修飾が担う
