...
`parse-core` は全実装必須の最小プロファイル (wire が宣言層である以上、parse の実行に lowering 実装を内包する)。他 3 つは opt-in。「kuu 準拠」を名乗る最小条件は `parse-core` green。descriptor の実装要求は `owns` (unknown-vocab 判定) と `reasons` (fixture の reason 検証) の 2 軸のみ — `observes` / configurable factory の一般適用は canonical 実装の装備であり準拠の要求ではない。

spec バンドル自体の v1.0.0 発行条件 (4 プロファイル全 green) は本節の実装準拠条件とは別軸 (DR-108)。
