...
- `argv_pos` は 0-based で、**失敗が帰属する argv トークンの位置**を指す。piece 単位の失敗 (piece_filters / type.parse / value_filters、DR-034 pieceProcessor) は piece が由来する値トークンの位置。**どのトークンにも帰属しない失敗は `argv.length`** を指す — トークンが尽きて要求が満たせない (= 次に要求した位置)、env / config 由来の値の失敗 (`argv: []` なら 0)、`final_filters`/`accum_filters` の reject (multiple 有無を問わず、確定した最終値・累積配列全体への一括検証であり特定トークンに帰属しない、DR-102 §4) がこれに当たる
