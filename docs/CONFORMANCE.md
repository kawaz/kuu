...
| `help` | help クエリを再現 (DESIGN §15.15) | `"help"` | `fixtures/help/` |

`parse-core` は全実装必須の最小プロファイル (wire が宣言層である以上、parse の実行に lowering 実装を内包する)。他 4 つは opt-in。「kuu 準拠」を名乗る最小条件は `parse-core` green。descriptor の実装要求は `owns` (unknown-vocab 判定) と `reasons` (fixture の reason 検証) の 2 軸のみ — `observes` / configurable factory の一般適用は canonical 実装の装備であり準拠の要求ではない。

spec バンドル自体の v1.0.0 発行条件 (5 プロファイル全 green、DR-112 §11 で 4 → 5 に改訂) は本節の実装準拠条件とは別軸 (DR-108)。
...
| `query` | ✓ | `"parse"` (本書 §2) / `"lower"` (lowering 断面、DR-070 — `installers` 列挙 (省略 = 全登録、順序非規範)、expect は DR-063 §3 の面構造を緩比較、順列検査は runner 組み込みで fixture に順列を列挙しない) / `"definition_error"` (静的検査、本書 §2「definition-error」、DR-082 — `parse_definition()` の返値をそのまま転用、`cases[].args` は不要) / `"complete"` (補完クエリ、本書 §4、DR-104) / `"help"` (help クエリ、本書 §5、DR-112) |
...
- `query:"help"` (§5) の help model は、`options` / `commands` / `positionals` の entries が**順序込み比較** (並べ替え適用後の定義順保存が規範 — DESIGN §14.6/§15.15。candidates の順序非依存 multiset 比較・interpretations / errors の集合比較と対照的に、順序それ自体が仕様)。その他のフィールドは構造等価 (省略 = default と等価)
...
## 5. help クエリ (`query: "help"`、DR-112)

`query: "help"` の fixture は、`definition` に対する help query (DESIGN §15.15) の入力 (`path` / `depth` / `category`) を与え、help model の期待値を検証する。**パース実行なし** — `args` は無い (complete と違い args_before も無い、純静的クエリ)。
...
- `cases[].path`: optional、省略 = ルート (`[]` と同値)。サブコマンドスコープの選択 (DESIGN §15.15)
- `cases[].depth`: optional、省略 = `"scope"`。`"all"` で全層再帰 (各 command entry に `scope` フィールドの再帰埋め込み)
- `cases[].category`: optional。指定グループの entries に絞る
- `expect.outcome`: `"help"` 固定。他フィールドは DESIGN §15.15 の help model の直訳 (`command_path` / `usage` / `description` / `description_long` / `epilog` / `commands` / `options` / `positionals` / `help_entry`)
- 不在 path / category の case は `{"outcome": "definition-error", "errors": [{"kind": "absent-ref"}]}` を expect する (DR-082 の definition-error 転用と同じ流儀)
- **比較規約**: `options` / `commands` の entries は**順序込み比較** — 並べ替え適用後の定義順保存 (DESIGN §14.6/§15.15) が規範であるため、candidates の multiset 比較 (§3) と対照的に順序それ自体が仕様。`positionals` も順序込み (定義順)。その他のフィールドは構造等価 (省略 = default と等価、§3)。多言語実装間で「同じ定義から同じ help 素材が同じ順序で出る」ことを機械検証する — 文言でなく素材を pin するから翻訳・整形の自由を奪わない
...
fixtures/help/                  help クエリ fixture (query: "help"、フォーマットは本書 §5 / DR-112 §11)
```
...
1. `definition` (wire form) を parse_definition に通す。`query:"parse"`/`"complete"`/`"help"` では definition-error になったらその fixture は fail (定義自体が不正) — ただし `query:"help"` の不在 path / category case (§5) は help query 自身が definition-error を返すことを expect する正常系。`query:"definition_error"` では逆に definition-error になることが期待される正常系 — `parse_definition()` の返す `DefError` 列を §2「definition-error」の `errors` 形へ射影し、args を使わず (DR-082 §1) そのまま `expect` と比較する
2. `query:"parse"` は各 case の `args` で parse を実行し、outcome を §2 の JSON 形へ射影する。`query:"complete"` は各 case の `args_before`/`args_after` で `complete()` を実行し、候補集合を §4 の `candidates` 形へ射影する。`query:"help"` は各 case の `path`/`depth`/`category` で `help()` を実行し、help model を §5 の形へ射影する (パース実行なし)
...
- DR-112 (help クエリ fixture format — §5 の正本、help model / プロファイル 4 → 5 改訂)
- LOWERING §C.5 (二段比較戦略)
