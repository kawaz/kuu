...
> 本書は値が args / env / config から結果オブジェクトに届くまでの処理順・データフロー・各段の入出力型を、
...
| 値源 | 入口 | 実体 |
|---|---|---|
| **CLI args** | matcher / exact が照合・消費 → 消費した生文字列、または DSL 固定値 (`:set:true` の "true")。0-token 効果 (update 等) は文字列を作らない | `string[]` |
