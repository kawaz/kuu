...
| **default** | ラダー最下段。固定値は typed internal `set(value)`、動的値は `Value` を返す `cell_fns` 呼び出しで供給する。`Sentinel` 出力の `default` / `empty` fn はこの席では definition-error `invalid-range`。type preset も固定値を差す (`flag` = false / `count` = 0) | `<cell_fns>` |

文字列値は出所によらず字句層 (§2) を通って型 `T` になる。`cell_fns` が返す型付き `Value` は通常の set operand として filter pipeline へ入り、null は共通 dispatcher が filter を呼ばず素通しする。`Sentinel` は `default` / `empty` で、対応する cell operation になる (DR-130 §3、DR-131、DR-114 §2 / §6.1)。

### 1.2 CLI 発火からセルへの適用

```mermaid
flowchart TD
  A["a. 発火 (CLI 面のみ)<br/>set 縮退形 / default・empty direct op / cell fn {fn,args}"]
  F["b. cell_fns 呼び出し<br/>(args, FnCtx) → Value | Sentinel<br/>Value は通常の set 経路、Sentinel は default / empty"]
...
- **a. 発火**: lowered 形は、値が確定済みの set 縮退形、`default` / `empty` の direct op、または DR-114 §6.1 の `{fn,args}` carrier を使う。`unset` は fn carrier である
- **b. cell fn 呼び出し**: `cell_fns` から fn を引き、args と統一 `FnCtx` を渡す。`incr` は `ctx.old` を読み old + 1、`unset` は null を `Value` として返す。`Value` は通常の set operand と同じ filter pipeline へ入り、null は filter を呼ばず素通しする。`Sentinel` は `default` / `empty` で対応 operation として ledger に積む。図の Value→ledger 矢印は §2 / §3.2 の通常 set 経路を矢印内に畳んだ表記である
...
| 5 | value_filters | `T → T` (要素単位) | 検証 + 変換: `in_range:1:65535`, `non_empty` … args は全て string (DR-009 / DR-114 §6)。cell に書かれる実値に適用するため、parser 出力、DSL 固定値、cell fn が返した `Value` は対象になる。null 要素は共通 dispatcher が filter を呼ばず個別に素通しする。`default` / `empty` Sentinel は値を書かないので対象が無い |
...
| `empty` | `{exact, link, effect:{op:"empty"}}` | `Sentinel(Empty)` に対応し、array / map / record を空にする | true | 0 |
...
3. `Value` の返り値は通常の set operand として `value_filters` / `final_filters` を通って cell へ適用する。null は filter を呼ばず素通しし、cell 適用時だけ committed=false の unset 状態へ変わる。`Sentinel` は `default` / `empty` operation として適用する
...
| `cell_fns` | `(args, FnCtx) → Result<Value | Sentinel, Reason>` | variant / default_fn の universal fn。greedy 面では direct op または `{fn,args}` effect carrier、default 席では遅延値供給として呼ぶ。Sentinel 住人は `default` / `empty` (DR-114 §1 / §4 / §6.1、DR-131) | `set` / `incr` / `borrow` / `env` / `uuid` / `computed` / `unset` (Value) / `default` / `empty` (Sentinel) |
...
`cell_fns` は共通 registry だが呼び出し席の出力型適合を保つ。default 席が受け入れるのは `Value` 出力の fn だけで、`Sentinel` 出力は definition-error `invalid-range` となる。したがって `unset` は default mode と effect mode の両方で使え、`default` / `empty` は effect mode 専用である。空配列を既定値にする場合は `default: []` と書く (DR-114 §2 / §7、DR-131 §7)。
...
| effect 適用 | direct set / `{op}` / `{fn,args}` → `(cell, operation, operand?) → cell + committed` | fn の `Value` は set operand、`Sentinel` は `default` / `empty` operation (DR-114 §2 / §6.1) |
