...
> **由来 DR**: DR-009 (3 段 / 7 stage) / DR-034 (separator は multiple 内属性) / DR-036 (multiple registry と collectors 統合) / DR-074 (number/bool canonical 字句) / DR-075 (int 値空間判定 + int_round) / DR-061 (configurable factory)

1 個の生文字列が型付き最終値になるまでの 7 段。各 filter は**純粋関数** (コンテキストなし、失敗は reason 付き reject)。

```mermaid
flowchart TD
  S1["1. raw string<br/>string"]
  S2["2. pre_split_filters (filters registry)<br/>string → string"]
  S3["3. separator 分割 (multiple.kind、multiple registry)<br/>string → string | string[] | {k,v}[]"]
  S4["4. type.parse / value_parser (types registry)<br/>(string) → T (要素単位)"]
  S5["5. filters ('each' は暗黙、filters registry)<br/>T → T (要素単位)"]
...
| 2 | pre_split_filters | `string → string` | 分割前の全体に適用: `trim`, `regex_match:^[a-z]+$` … |
| 3 | separator 分割 | `string → string | string[] | {k,v}[]` | `--tag a,b,c` → `["a","b","c"]`。separator は multiple プリセットの属性 (DR-034 / DR-036) |
...
| `filters` | `validate: T → T | reject` ／ `transform: T → T` | `pre_split_filters:` / `filters:` / `post_filters:` / **update の transform (DR-077)** / 旧 collectors も統合済 (DR-036) | `trim` / `in_range` / `non_empty` / `increment` / `to_set` / `from_entries` |
...
| pre_split_filter | `string → string` | reject (kind=filter) |
