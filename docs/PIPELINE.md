...
> **由来 DR**: DR-009 (3 段 / 7 stage) / DR-034 (pieceProcessor = pre → parse → post、separator は multiple 内属性) / DR-036 (multiple registry と collectors 統合) / DR-062 (pre_filters への改名 + 継承二形) / DR-074 (number/bool canonical 字句) / DR-075 (int 値空間判定 + int_round) / DR-061 (configurable factory)

1 個の生文字列が型付き最終値になるまでの 7 段。各 filter は**純粋関数** (コンテキストなし、失敗は reason 付き reject)。段 3〜5 は piece 単位 (DR-034 の pieceProcessor)。multiple 無しの要素は separator なしの長さ 1 縮退として同じ管を通る (DR-034 §6.3 相当)。

```mermaid
flowchart TD
  S1["1. raw string<br/>string"]
  S2["2. separator 分割 (multiple.kind、multiple registry)<br/>string → piece[] (single は長さ 1)"]
  S3["3. pre_filters (filters registry)<br/>string → string (piece 単位)"]
  S4["4. type.parse / value_parser (types registry)<br/>(string) → T (piece 単位)"]
  S5["5. filters ('each' は暗黙、filters registry)<br/>T → T (piece 単位)"]
  S6["6. accumulator (multiple.on_repeat、accumulators registry)<br/>(Acc, T) → Acc"]
  S7["7. post_filters (filters registry)<br/>Acc → Acc"]
...
| 5 | filters | `T → T` (要素単位) | 検証 + 変換: `in_range:1:65535`, `non_empty` … args は全て string (引数パースと同じ手順、DR-009)。`filters` が乗る対象は cell に書かれる実値のみ — set operand / update 適用結果 / 値源席 (env・config・inherit) の供給値 (DR-049/050)。cell 操作 op (unset/default/empty) の発火は値を書かないので `filters` の適用対象が無い (DESIGN §8.3、`pre_filters`/`post_filters` は位相が異なる — 同節参照) |
...
4. 畳み後、6〜7 段 (accumulator / post_filters)
...
| `filters` | `validate: T → T | reject` ／ `transform: T → T` | `pre_filters:` / `filters:` / `post_filters:` / **update の transform (DR-077)** / 旧 collectors も統合済 (DR-036) | `trim` / `in_range` / `non_empty` / `increment` / `to_set` / `from_entries` |
...
| pre_filter | `string → string` (piece 単位) | reject (kind=filter) |
...
| post_filter | `Acc → Acc | reject` | reason (kind=filter) |
