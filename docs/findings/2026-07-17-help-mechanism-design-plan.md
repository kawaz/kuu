...
> **hidden の語彙裁定 (HELP-Q12=a、kawaz 2026-07-18): `hidden: bool` 1 本を維持。** 9 系統実測で bool 1 本が圧倒的多数派 (段階制 visibility は Swift AP のみ、hide_short_help 型の部分 hide は clap のみ)。clap の「-h では隠すが --help では出す」相当の非対称が欲しい場合は、**kuu では ref で分割定義すれば良い** (kawaz 裁定理由) — 同じ実体を指す ref 要素を hidden 有無違いで 2 つ立てれば面別の見せ方が既存機構で組める。面別 hidden 語彙 (`hidden: ["help"]` 等) は導入しない。
