...
outcome を受ける — **ResolvedOutcome を受理する形に置く** (M2c 監査による訂正 2026-07-24:
当初の「parse 相で確定する発火」は誤り。#completion_script cell は default_fn / env / preset
経由の充填が値源ラダー相で確定するため、parse 相では last-wins の素材が揃わない — 実物確認済み))。
...
| `Interpretation` | `@engine.AmbiguousInterpretation` | opaque + 射影メソッド | `output_of_interpretation(ast, interp)` (実装採用名 — 既存玄関名の踏襲) によるビュー射影と `resolve_interpretation(ast, interp, ...)` への受け渡し。claimants は `claimants()` で `Array[(String, String)]`。ast との対応は doc 契約 (AP2-Q4=b、§1.2b) |
...
| `ParseError` | `@engine.ParseError` | pub struct | wire 正本 (schema/fixture.schema.json `runtimeError`) どおり `{element, args_pos, kind, reason, message, path}` — `path : Array[String]` は非 optional で **空配列 = scope 修飾なし** (wire の `path?` 省略と空配列が構造等価、CONFORMANCE §3 の省略 = default 規約)。実装の現行フィールド名 `args_pos` / `path` を保持 (sol major 11: 当初表の `argv_pos` 表記は DR-053 起草時の旧名で誤り、`path` の脱落も訂正) |
...
- **API 形状は ast 引数維持**: `output_of_interpretation(ast, interp)` /
...
| M9 | variant × API の遷移表欠落 | 採用 — 表を追加 (dispatch_completion_script は当初 ParsedOutcome 受理としたが M2c 監査で ResolvedOutcome 受理に訂正 — §1.1b 注記) | §1.1b |
