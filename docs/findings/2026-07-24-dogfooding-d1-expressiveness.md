...
| 分類 | 発見 | 一言 |
|---|---|---|
| 裁定候補 (spec 拡張・裁定が要る) | F1 | 頻出 CLI パターンの宣言席が無い、または規定の衝突 |
...
### F5 (裁定済み DOG-Q4γ=a, 2026-07-24): 「引数なし = help 表示」は宣言席不要 — アプリの裁量

> **裁定**: 宣言席 (help_on_empty 等) は設けない。help がユーザに届く経路は
> (1) 成功時: result の help キーをアプリが見て renderHelp、(2) 失敗時: fired_action を
> アプリが見て renderHelp、の 2 本であり (DR-113 §2 / DR-048)、「引数ゼロで help を出す +
> exit code を 0 にするか 1 にするか」はアプリの裁量。definition が関与する話ではない。
> kuu-cli D2 の実証: dispatch 冒頭の args 空特判 1 分岐で足りた (commit 1820187d)。
