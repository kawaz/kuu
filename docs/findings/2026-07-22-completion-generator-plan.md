...
> さらに query 契約の最終形が確定 — **`KUU_COMPLETE=<UUID>` env と argv 第一引数の
> UUID 二箇所一致**で「モード切替」と「自分宛の指示であること」を同時判定する
> (kuu 製コマンドを completer が内部で呼ぶ自己参照シナリオへの機構的防御、§3.1)。
> 2026-07-23 に GEN-Q3''/Q4'''/Q5''/Q6'' = 全て a で確定し **GEN-Q バッチは全消化** (§9 が
> 裁定記録)。本版は確定骨格 — **生成 = help 同型 preset (定義者の自由) / query = UUID
> 二箇所一致の env モード切替 (定義非侵襲)** — の最終版であり、次フェーズは ABI DR 起草。
...
- **query 口は env var モード切替、definition 非侵襲** (GEN-Q4' 棄却の帰結):
  ユーザの引数定義は無限の自由度を持つため、その中に予約綴り (サブコマンドや option) を
  衝突なく仕込めるという前提自体が誤り — 引数には触らないのが一番安全。最終形は
  **`KUU_COMPLETE=<UUID> <セルフバイナリ> <UUID> <SHELL> <words...>`** — env 値と argv
  第一引数の **UUID 二箇所一致**で「モード切替」と「自分宛の指示であること」を同時判定
  する。一致したら argv の解釈を一切せず補完 query モードに入り、不一致 (env 継承のみ)
  なら通常実行に倒れる (§3.1 に自己参照シナリオと共に詳述)
...
|---|---|---|---|
| 表現形 | type preset 5 種 | type preset 1 種: `completion_script` | **preset なし** — UUID 二箇所一致の env モード切替 (definition 非侵襲、§3) |
...
| 入口 | long / short / env、`or` 合成、どの scope でも | 同じ (定義者の自由) | 入口自体が argv 文法の外 (env + argv[1] の UUID 一致) |
| 値スロット | `help_category` = bool 枝 + string 枝の or | string (shell 名、必須値) | argv[2] = shell 名、argv[3..] = words |
...
- shell 名の値域は spec では閉じない — 対応 shell 集合は生成器実装 (product) の関心で、
  ...
  script 生成 capability の実行時エラー
- **受理は自由入力、補完候補は提示する** (GEN-Q3'' = a の追加要件、kawaz 裁定
  2026-07-23): preset は **values enum を張らない** — 縛ると未対応 shell 名の実験や
  拡張が不便になる。一方この値位置の**補完候補としては実装対応 shell 名 (zsh / bash /
  fish + 実装依存の追加分) を提示する**。「shell 名候補を補完で提示するが受理は自由
  入力」の糖衣であることを preset 定義 (ABI DR) に明記する
- `on_failure` 既定 false
...
2. **補完対象コマンド名** (shell の補完登録 `compdef` / `complete -F` が要求) —
   program_name (§2.2)
3. **UUID** (§3.1 の二箇所一致トークン) — script 生成時に採番して焼き込む。補完のたびに
   生成するのではない

## 3. 論点 3: query 契約 — `KUU_COMPLETE` UUID 二箇所一致の env モード切替

### 3.1 確定形 (kawaz 裁定 2026-07-22〜23)

```
KUU_COMPLETE=<UUID> <セルフバイナリ> <UUID> <SHELL> <words...>
```

- **UUID 二箇所一致判定**: ux 層 runtime の玄関 (parse 呼び出しの入口) は argv 解釈前に
  `env["KUU_COMPLETE"] == argv[1]` (完全一致のみ) を判定する。一致したら **argv の解釈を
  一切せず**補完 query モードに入り、`argv[2]` = shell 名、`argv[3..]` = 補完対象の
  words として橋渡しに専念する。不一致 (env 未設定を含む) なら通常実行
- **UUID の目的は秘匿ではなく自己宛 identification** — 「モード切替」と「これはあなた
  だけへの指示である」の同時判定。ps で見えて構わない (機密性は要求しない)。UUID は
  glue script の**生成時に採番して焼き込む** (補完のたびに生成するのではない — §2.3)
- **不一致時の安全性**: env 継承だけで UUID がある子プロセスは argv に UUID が無いので
  通常実行に倒れる。逆に argv に UUID 様の文字列があっても env と一致しなければただの
  引数 — 定義に無い第一引数として通常の parse エラーに落ちるだけで、誤動作の余地が無い

**なぜ env 単独判定では足りないか (kawaz 提示の自己参照シナリオ)**: `ls` が kuu 製だと
する。`KUU_COMPLETE=... ls <UUID> zsh ...` の補完 query 実行中に、positional の paths を
解決する custom completer が内部で `ls` を呼ぶと、env 継承だけの判定では**子の ls も
補完モードに入って壊れる**。この穴を塞ぐ代替案はどれも決め手を欠く (「採用しなかった
案」§3.6 参照) — UUID 二箇所一致は「親 glue が意図した宛先だけがモードに入る」ことを
実装の行儀 (unset の徹底など) に依存せず**機構で**保証する。

モード切替の判定は clap の `CompleteEnv::complete()` (main 冒頭の手動仕込み) と違い、
kuu は ux 層が玄関 API (definition + argv → outcome) を持つため玄関内部で判定でき、
アプリ開発者に追加の仕込みは要らない — clap の「main 冒頭に書き忘れる」縁は ux 層
設計で吸収できる (各言語 ux 層 API の関心)。
...
**`KUU_COMPLETE_INDEX=<N>` (0-origin、words = `argv[3..]` 内のカーソル単語 index) を
推す**。argv 内に `--cword N` 等の制御トークンを混ぜる案は「UUID・shell 名以降は純粋に
words」という確定形 (§3.1) の純度を壊すため採らない。省略時は行末補完 (カーソル =
words 末尾の次、args_after 空) とみなす — glue は常に INDEX を付けるが、手で叩く
デバッグ時に省略できる縁を残す。
...
binary 側の分解 (§0.1-1 の complete API への写像。words = `argv[3..]`):

- `words[0..cword]` → `args_before` (words 先頭に補完対象コマンド名トークンを含めるか
  等の細部は ABI DR で確定 — glue が shell words の `[1]` 以降だけ渡すか、全量渡して
  binary が先頭を落とすかは glue テンプレと対で決める)
...
- **子プロセスへの env 伝播**: UUID 二箇所一致 (§3.1) が機構で解決済み — env が子へ
  継承されても argv に UUID が無ければ通常実行に倒れる。unset の徹底 (実装の行儀) に
  依存しない。query モード中の unset は「やっても害は無い衛生」程度に格下げされ、規約
  ではなくなった (§3.6 の棄却経緯参照)
- **手で叩くデバッグ口**: `KUU_COMPLETE=x app x zsh <words...>` (env と argv[1] に同じ
  文字列を書く) で誰でも query モードを手動起動できる — UUID は自己宛 identification で
  あって秘匿ではないので、デバッグ時は任意の一致文字列でよい。cobra `__complete` の
  「手で叩ける」利点は UUID 方式でも保たれる。env と argv の不一致は通常実行になる
  だけで安全 (clap の panic のような破壊的誤発火は無い)
...
- 幻影コマンド用の glue script には `kuu completion query /path/def.json --shell zsh
  ...
  形は元々テンプレ変数 (形態 A = `KUU_COMPLETE=$uuid "$prog" $uuid zsh ...`、形態 B =
  上記) であり、glue の固定依存は応答行形式 (§3.3) だけ — テンプレは両形態で共有できる。
  形態 B に UUID が無いのは、`kuu completion query` が正規サブコマンドで「自分宛か」の
  曖昧さが生じない (kuu-cli 自身の argv 文法の内側) ため
...
### 3.6 採用しなかった案 — env 単独判定の穴埋め (自己参照シナリオで全滅)

kawaz 提示の自己参照シナリオ (§3.1 — kuu 製 `ls` の補完中に completer が内部で `ls` を
呼ぶ) に対し、UUID 二箇所一致に至る前に検討・棄却された案:

- **env 単独判定 (4 版目当初案: `KUU_COMPLETE=<shellname>` のみ)**: env は子プロセスへ
  継承されるため、completer が呼ぶ子の kuu 製コマンドも補完モードに入って壊れる。棄却
- **unset 規約 (4 版目当初案の緩和策)**: query モード突入時に自 env から `KUU_COMPLETE`
  を unset する規約。ux 層実装の行儀に依存し、unset 前に spawn される子・ux 層を経由
  しない子プロセス起動 (ホスト言語の生 exec) を防げない。機構でなく規約なので保証に
  ならない。棄却 (害の無い衛生としてやるのは自由)
- **realpath 照合 (env 値にバイナリパスを入れて自身と照合)**: 補完対象と同一バイナリを
  completer が内部で呼ぶ自己参照ケース (`ls` の completer が `ls` を呼ぶ) では realpath
  が一致してしまい、まさに守りたいケースで守れない。棄却
- **argv 単独のマーカー (env なし、argv[1] = UUID のみ)**: 「UUID らしき第一引数」だけ
  ではモード切替の意図を持たない通常引数と区別できず、誤発火の判定根拠が無い。env
  (モード切替の意図) と argv (宛先の特定) の**二箇所**が揃って初めて「あなただけへの
  指示」になる。棄却
...
**prefix 絞りの挙動 (kawaz 裁定 2026-07-23)**: 前方一致で候補がゼロになった場合、
**空リストを返す** — 入力済みのカーソル単語はそのまま残る (例: `--choice` の値が
`foo|bar|baz` の 3 択で `--choice c<TAB>` → 候補なし、`c` は消えない。<TAB><TAB> でも
同じ)。裁定理由: ユーザが入力中の `c` に何か意味があるかもしれないのに勝手に消すのは
ユーザの意図に反するリスクが高い。対案 B (一致ゼロなら入力を消して全候補を出し直す)
は不採用 — B の実現自体も bash では標準枠組みの外に出る (§4.3 の bash 行の注記)。
...
**bash の書き換え能力の正確な整理** (kawaz 指摘 + 実機確認済み 2026-07-23): 「bash は
カーソル単語の置換しかできない」という通説は不正確。正しくは — bash でも
`READLINE_LINE` / `READLINE_POINT` (`bind -x`) を使えば**ライン全体の取得・書き換えは
可能**。ただし **`complete -F` の標準 programmable completion 枠組みの中では `COMPREPLY`
による現在単語の置換のみ**が表現手段であり、「入力中の単語を消して全候補を出し直す」級の
ライン書き換え (§4.2 prefix 絞り裁定の対案 B) を実現するには TAB キー自体を `bind -x`
で乗っ取る非標準 glue が必要になる。それは既存の bash-completion エコシステム (他
コマンドの補完定義) と共存できないため、kuu の glue は標準枠組み (`complete -F` +
`COMPREPLY`) の内側に留まる。
...
1. **ABI DR (spec 側の新 DR)**: (a) `completion_script` preset の canonical 展開 (入口・
   ...
   (`KUU_COMPLETE` の UUID 二箇所一致判定・argv 配列 (UUID / shell / words) の構造・
   `KUU_COMPLETE_INDEX`・玄関判定が argv 解釈に先行すること・UUID は自己宛 identification
   で秘匿でないこと・不一致 = 通常実行)、(c) 応答行文法 (タブ区切り・候補フラグ・
   directive 語彙・エスケープ)、
...
- **既存決定・裁定の追認** (新規裁定なし): ブリッジ経路 (GEN-Q1 = a) / 生成 = preset・
  query = UUID 二箇所一致の env モード切替の骨格 (GEN-Q3'/Q4' 裁定と query 最終形の
  kawaz 確定) / policy の中身 (DR-116 §2〜§5) / 素材とポリシーの分離 (DR-060 §3) /
  builtin completer の shell 機能委譲 (DR-060 §4) / 出力非 pin (DR-116 §6) / preset
  パターンの型 (DR-113 §2)
- **本 findings の発明 (GEN-Q 裁定対象または ABI DR 確定事項)**: (1) `completion_script`
  ...
  per-candidate nospace) (§3.3)、(4) 手動デバッグ起動の整理 (任意の一致文字列で可、
  §3.4)、(5) 形態 B は env でなく正規サブコマンドで capability 露出 + UUID 不要の理由
  (§3.5)、(6) word の消費位置 = 生成器 prefix 絞り段 (§3.2)、(7) 置き場所 3 層と
  conformance 増分の再整理 (§5)、(8) 形態 B の custom completer 縮退規則 (§6)
...
- **conformance 増分の発生** (§5.2): preset 化は「実装課題だけ、spec 静穏」だった本 issue
  ...
  構想比で半減)
- **env プロトコルの ux 層規約依存**: 「玄関で UUID 二箇所一致を判定して argv 解釈を
  奪う」は全言語 ux 層が正しく実装して初めて成立する規約 (spec fixture では観測しに
  くい runtime 挙動)。ux 層 conformance の検証手段 (product test の共通シナリオ集等) を
  ABI DR の波及に書く必要がある。UUID 一致判定自体は 1 比較なので実装差は出にくいが、
  「不一致 = 通常実行 (エラーにしない)」の一様性は明文で縛る必要がある
...
## 9. GEN-Q バッチ — 全消化 (裁定記録)

| ラベル | 質問 (1 行) | 裁定 |
|---|---|---|
| GEN-Q1 | 生成器アーキテクチャ | **a 確定** (kawaz 2026-07-22): ブリッジ型一択。静的展開はシェルで kuu を再実装する複雑さになる |
| GEN-Q2 | v1 対象 shell | **a 確定** (kawaz 2026-07-22): zsh / bash / fish (PowerShell は追随) |
| GEN-Q3'' | 生成側入口の preset 形 | **a 確定** (kawaz 2026-07-23): `type: "completion_script"` — shell 名を必須値に取る string preset (bool 枝なし)、shell 名の値域は spec で閉じない (未対応は capability 実行時エラー、DR-111 §6 と同じ線)、`on_failure` 既定 false、入口は定義者の自由。**追加要件**: values enum は張らない (未対応 shell 名の実験・拡張を縛らない)、この値位置の補完候補として実装対応 shell 名を提示する糖衣であることを preset 定義に明記 (§2.2)。付帯論点 (positional 入口を help preset と揃えて許すか) は ABI DR 起草時に確定 |
| GEN-Q4''' | cword (カーソル位置) の受け方 | **a 確定** (kawaz 2026-07-23、GEN-Q 全消化の一部): `KUU_COMPLETE_INDEX=<N>` (0-origin) の別 env、省略時は行末補完扱い。argv 内制御トークンは words 純度を壊し、cobra 末尾方式は args_after (after 整合フィルタ) が原理的に運べない (§3.2) |
| GEN-Q5'' | custom completer が実行できない場合の縮退 | **a 確定** (kawaz 2026-07-23、同上): 形態 A では問題自体が不存在。形態 B に限り「候補提供なし + validate 面で要求 capability を機械可読報告」、files への fallback はしない (§6) |
| GEN-Q6'' | 正本と実装の置き場所 | **a 確定** (kawaz 2026-07-23、同上): preset 語彙 + env プロトコル + 応答行文法 + capability 概念シグネチャ = spec の新 ABI DR (DR-113 対称。DR-111 §5 の io_type 軸は先送り明記で座席維持) / glue テンプレ = 言語非依存の共有資産 / capability 実装 = 各言語 ux 層 (一番手 MoonBit)。conformance 増分 (schema type 1 種 + lowering / definition-error fixture) が発生するため発題 issue の受け入れ条件更新が必要 (§5.2) |
