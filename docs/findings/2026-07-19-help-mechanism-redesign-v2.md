...
### 4.4 default_fn 一本化 (kawaz 追補 mid=28 + mid=32) — default 席の unified 意味論

kawaz mid=28 追加提案の裁定 + mid=32 命名補正: **default 席の値は常に default_fn 経由**、`default: value` / `env: "VAR"` / `inherit: true` は全て default_fn の糖衣として整理:

| 糖衣記法 (wire form 不変) | 展開後 default_fn |
|---|---|
| `default: value` | `default_fn: "set:<value>"` (Q8=A 統合で variant DSL `":set:X"` と座が同じ = 別名不要、mid=32) |
...
**fn 命名**: **`set`** を採用 (Q8=A universal fn 統合で variant DSL `":set:X"` と座が同じ = 別名不要、mid=32 kawaz 指摘で `constant` → `set` に逆訂正)。
