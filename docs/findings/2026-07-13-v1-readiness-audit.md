...
- V1-R02 evidence 内で発見された **DESIGN.md:1132 の stale 記述** (「Schema ファイルのドラフト書き出しは残作業」← 実際は実体化済み) は要件充足自体に影響しないが、既に完了した作業への言及であり最新を読む読者にはノイズなので削除対象。V1-R11 の転記作業と合わせて修正するのが効率的。
