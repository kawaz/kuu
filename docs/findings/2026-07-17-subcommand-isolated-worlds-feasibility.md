...
## kawaz の追補 (2026-07-17、検討報告への応答)

> 「完全別世界と言ってもグローバルオプションは共有するしサブコマンド分は json キーが一つ深くなるなどの融合はあって然るべきでしょう。」

構想の「別世界」は完全隔離ではなく**融合面を持つ別世界**: (1) グローバルオプションの共有 (= global installer の構造コピーは意図された越境として温存)、(2) 結果の JSON キーが subcommand 分 1 段深くなるネスト (= 現行の scope ネスト射影がそのまま)。本 findings の「主価値は名前空間の衛生、global 越境は唯一の正当な越境」という整理と一致し、下記懸念 1 の「看板を下げる」は「完全隔離ではないと明示する」形で kawaz 自身が確認した。
