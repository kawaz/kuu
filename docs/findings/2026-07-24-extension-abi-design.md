...
> 再発明) で §8.8 第 4 版 (公開 ABI を (A) 面に絞る + AP2-Q5 起票) → **kawaz 裁定が
> 二択を枠組みごと否定 (「Node/Ctx をどう公開するかを考えよう」) → §8.10 第 5 版 (現行):
> Node/Ctx の公開面を正面設計 — Branch/Binding は abi、Ctx/Resume/NodeExt/MatcherExt/
> InstallerExt は extension、Node は internal ADT のまま kuu-node package が構築子 +
> typealias を公開。実験 12 で 4 顧客 (自作 node resident 込み) green** → §8.11 第 5.1 版:
> InstallerExt のみ kuu-node 居住へ訂正 (extension 居住は cycle、実験 13 で green)。
> §2 の型リストは §8.10.3 + §8.11.2 の表が最終正。
...
- **(A) 拡張作者と同じ立場で書ける面**: type residents / completer / accumulator /
  collector / filter descriptor / cell_fns / entity_ext — grep 実測で **Node 参照ゼロ**
  (builtins 14 ファイル中 10 ファイルが該当)
- **(B) engine の open node 契約 (DR-110 §3-2) に乗る評価能力の提供面**: node residents
  (値プリミティブの読み/parse を NodeExt::eval で登録、DR-110 §4「値プリミティブ型は
  open node 契約に乗る拡張 node」)、installer residents (Definition/ElementDef を読む
  lowering 参加者)、**matcher residents も (B)** (実装列挙後の訂正: matcher_residents.mbt
  は `@engine.Branch` / `Ctx` / `Candidate::trigger` に依存 — MDR-002 §3 の自己申告どおり
  node.mbt 定義の Branch を返す評価器参加者で、Node 非依存でも evaluator 座標依存)。
  これらは**評価器の内部座標 (Ctx/Branch/Binding/Resume/Node) をシグネチャに持つ trait の
  実装**であり、wire に現れない評価器 ABI の住人

実装側の停止条件列挙 (M2c 2026-07-24) がこの切り分けの規模感を確定させた: builtins 公開
mbti の Node 戻り 15 口中 13 口 (非公開化対象 2 口を除く全部)、内部 NodeExt resident 12 種
(StringArg / Kv / Separated / NumberArg / FilteredNumberArg / Float / Int / Bool / Typed /
EffectMark / DeprecationMark / FailureMark)、enum_value_node は structural Or/Seq/Exact/Bind
を要求。abi plant で代替するには 12 resident の eval / encode / element_name / zero-token /
effect / failure 意味論 + 構造 4 種の再表現が必要 = **Node 再発明そのもの** (発火の裏付け)。
...
- **builtins を 2 分割**:
  - `src/builtins/` (public、import = abi + extension のみ): (A) 面 — type/completer/
    accumulator/collector/filter/cell_fns/entity_ext の 10 ファイル。**ここが「拡張作者と
    同じ書き方」の対称性の証明** (moon.pkg に internal が無いままコンパイルが通る)
  - `src/internal/residents/` (internal、import = abi + extension + internal/engine):
    (B) 面 — node_residents + installer_residents + **matcher_residents** (+ lexicon の
    ElementDef 依存部)。canonical 資材として evaluator 型に直接触る。NodeExt resident
    12 種と matcher 2 種 (eq_split / short_combine) の意味論は一切書き換えない — 移動のみ
...
- [ ] matcher_residents.mbt も internal/residents へ (§8.8.1 訂正の反映)。matcher 構築 4 本
      (eq_split / short_combine / eq_entry / short_entry) の production 呼び出し元は玄関
      registry 組成 (src/kuu/registry.mbt) — 玄関は internal import 可なので移動で足りる。
      §3-1 の「matcher 構築 4 本は公開維持」は superseded
- [ ] AP2-Q5 の裁定を待ってから extension の公開 trait 面を fix (a なら上記どおり、
      b なら BuilderView 設計を §8.4 の停止条件込みで再開)

#### 8.8.5 中間状態 (dirty 37 files) からの継続可否 — 継続可、仕切り直し不要

M2c の中間変更 (kuu.mbt working copy、jj st で確認) と第 4 版の突き合わせ:

| 中間変更 | 第 4 版での扱い |
|---|---|
| `src/engine/` → `src/abi/` の純データ移動 (value / result_value / invocation / warning / node_types) | **そのまま生きる** (§8.2 の abi 層そのもの) |
| `src/engine/` → `src/extension/` の移動 (registry / capability / cell_fn / filter / fn_ctx / accumulator_ext / completer_ext / node_traits の SeatCtx) | **大半生きる**。Registry からは register_installer / register_matcher / register_node を internal 側組成 API へ分離する差分のみ (§8.8.2)。installer_ext.mbt が internal/engine に居るのは第 4 版と一致 (extension へ移さない — 正しい) |
| `src/engine/` → `src/internal/engine/` の移動 (node / eval / cont / lowering ほか) + installer.mbt → lowering.mbt | **そのまま生きる** (inst_* の internal 移動は §8.8.1 (B) の先取り) |
| `src/builtins/moon.pkg` の import = abi + extension (`@engine` alias で extension を指す) | (A) 面 10 ファイルには**そのまま正**。(B) 面 3 ファイル (node/installer/matcher residents) を internal/residents へ抜けば moon.pkg は変更不要のまま通るはず |
| `src/extension/node_traits.mbt` / `src/abi/node_types.mbt` の新規 | node_traits の SeatCtx は生きる。**NodeExt / MatcherExt 相当を extension に置いた部分があれば internal へ戻す** (実装確認事項)。abi/node_types.mbt は DefError 系なので命名だけ誤解を招く — 中身は abi で正 (rename は任意) |

**結論: 仕切り直し不要**。残作業 = (1) builtins から (B) 3 ファイルを `src/internal/residents/`
へ移動 + moon.pkg 新設、(2) 公開面から factory 13 口 + matcher 構築 4 本を落とす (internal
居住へ)、(3) extension から evaluator 座標 trait を internal へ戻す (該当があれば)、
(4) Registry の register 口分離。いずれも移動 + 可視性変更で意味論の書き換えゼロ。

### 8.10 第 5 版 — AP2-Q5 の再定式化を受けた Node/Ctx 公開面の正面設計

> kawaz 裁定 (2026-07-24): 第 4 版の AP2-Q5 a/b 二択は枠組みごと否定 —「Node/Ctx なしにやる
> のは無理。それをどう公開するかを考えようという話」。v1 で node / installer 拡張まで公開する
> 前提で公開面を設計する。第 4 版の「(B) 面は internal 閉鎖」は撤回、**2 分割の観察
> ((A)/(B) の依存差) と中間状態の継続可否 (§8.8.5) は有効なまま** — 変わるのは (B) 面の
> 契約に公開形を与えること。

#### 8.10.1 必要面の全数列挙 (node resident 12 種 + matcher 2 種の grep 実測)

node_residents.mbt の評価器 API 接触 (出現数付き、機械集計):

| 接触面 | 出現 | 分類 |
|---|---|---|
| `Held` / `Pending` / (matcher で `Accept`) | 18/9/7 | **Branch の 3 variant 構築** — 全成分が wire 級純データ (ParseError / Candidate / Binding / Int) |
| `ctx.token_at` / `ctx.extensions` | 9/8 | **Ctx の観測 2 面** (+ matcher は is_complete_mode) |
| `Binding::new` / `::effect` / `::deprecation_mark` / `::action_marker` | 8/1/1/1 | **Binding の公式構築子 4 本** |
| `Candidate::pending_value` (+matcher で `::trigger`) | 9 | Candidate 構築子 2 本 |
| `ParseError::filter` | 7 | ParseError 構築子 |
| `apply_piece_filters` | 7 | filter chain 実行ヘルパ (Registry + FnInvocation + String → Result) |
| `FnInvocation` / `TypeExt` / `RoundMode` / `BoolConfig` / `EffectOp` / `Value` variant | 17/5/5/5/3/6 | 既に abi / extension 居住確定分 |
| `Ext(... as &NodeExt)` | 13 | **Node の Ext variant への注入** (= open node 契約の座席) |
| NodeExt メソッド実装分布 | kind/encode/consumes_zero_tokens/eval 各 12、element_name 9、is_failure_action 1、effect_mark 1 | 必須 4 + optional 3 (default 実装持ち) |

enum_value_node (lowering.mbt:763) の structural 消費: `Or` / `Seq` / `Exact` / `Bind` の
4 構築のみ (分解なし)。matcher_residents の追加接触: `Branch` 3 variant + `Ctx` +
`AttachMode` / `Require` / `LastOnly` / `Deny` (short 系 config enum、純データ)。

**観察**: 拡張が要る面は (1) 純データの構築 (Branch/Binding/Candidate/ParseError)、
(2) Ctx の観測 3 面、(3) NodeExt trait の impl、(4) Node の構築 (structural 4 + Ext 注入)
— **Node の分解 (variant match) はどの resident も行わない** (evaluator の専権)。つまり
公開すべきは「構築と観測」であり ADT の露出ではない — 再発明でない最小安定面が存在する。

#### 8.10.2 実験 12 (probe、moon 0.1.20260709) — 公開形の実機確定

構成: abi (Branch を含む純データ + private fields/構築子) ← extension (Ctx opaque 窓 +
`pub typealias Resume` + **NodeExt trait 本体** + Registry) ← internal/engine (Node ADT:
Exact/Bind/Seq/Or/**Ext(&@ext.NodeExt)** + evaluator 全面 match) ← **factory (public、
internal を import し structural 構築子 5 本 + `pub typealias @eng.Node as Node` を公開)**
← 外部 consumer。

- moon check 全 green。**外部 consumer が (a) 自作 NodeExt resident (独自値プリミティブ
  HexArg) を impl、(b) structural 合成で enum_value_node 相当を自組み (or(seq(exact,bind)))、
  (c) `@factory.Node` の typealias で型注釈も書ける** — 全部 green
- 判明した制約 2 つ: (i) 外部は internal 型名を直接書けない (`Package "eng" not found`) —
  **public package の `pub typealias` が名指し面を提供する** (値の pass-through は従来どおり
  自由)。(ii) Branch は全成分が純データなので abi 居住が成立 — Resume も
  `(Ctx, Int, Array[Binding]) -> Array[Branch]` の typealias で extension に置ける

#### 8.10.3 第 5 版の公開面設計

| 公開面 | 居住 | 形 |
|---|---|---|
| `Branch` (Accept/Held/Pending) | abi | pub(all) enum (成分は private-field struct)。**評価プロトコルの安定 3 値** — variant 追加は open node 契約の major |
| `Binding` / `Candidate` / `ParseError` | abi | pub struct (private fields) + 公式構築子 (`Binding::new`/`effect`/`deprecation_mark`/`action_marker`、`Candidate::trigger`/`pending_value`、`ParseError::filter`/`constraint`) + read getter |
| `Ctx` | extension | opaque 窓: `token_at` / `is_complete_mode` / `extensions` の観測 3 面 + internal 専用構築子 (doc 契約「評価器のみが構築する」) |
| `Resume` | extension | `pub typealias (Ctx, Int, Array[Binding]) -> Array[Branch]`。node.mbt 既存の doc 契約 (「現 eval 呼び出しの継続としてのみ使用、保持禁止」) を移設 |
| `NodeExt` | extension | pub(open) trait — 必須 4 (kind/encode/consumes_zero_tokens/eval) + optional 3 (element_name/is_failure_action/effect_mark、default 実装)。**現シグネチャのまま全成分が abi+extension 型になる** (§8.10.1 の列挙どおり) |
| `MatcherExt` | extension | 同様に成立 (interpret の Ctx/Branch が公開面に乗ったため)。short 系 config enum (AttachMode/Require/LastOnly/Deny) は abi の config 担体へ |
| `Node` | **internal 定義のまま** | ADT は動かさない (evaluator match 無変更)。公開は **kuu-node package (public) の構築面**: structural 4 構築子 (`or`/`seq`/`exact`/`bind`) + `ext_node(&NodeExt)` + arg factory 14 本 + `pub typealias Node` (名指し面)。**分解 API は置かない** — 「構築はできるが分解できない」を typealias + 構築子の組で実現 (実験 12c) |
| `apply_piece_filters` 等の実行ヘルパ | extension | resident が filter chain を回す口 (Registry + FnInvocation + String → Result[Value, FnReason] — 全部公開型) |
| `InstallerExt` | **kuu-node** (§8.11 第 5.1 版で訂正 — 当初の extension 居住は cycle) | InstallBuilder / DefsView の観測・植え付けは kuu-node の窓 fn + typealias 経由。実験 13 で外部 installer 拡張込み green |

**package 構成 (最終形)**: `abi` ← `extension` ← `internal/engine` ← `kuu-node` (factory,
public) / `builtins` (public、(A) 面 + node/installer/matcher residents **も戻す** — residents
は extension trait と kuu-node 構築子だけで書けるようになったため第 4 版の 2 分割は不要。
builtins の import = abi + extension + kuu-node、internal 直 import なし = **DR-110 の機械証明が
(B) 面込みで成立**) ← `kuu` (玄関)。

#### 8.10.4 4 顧客の机上検証

1-3 (bigint / custom completer / 自作 type): 第 4 版から変わらず書ける (公開面は広がる方向のみ)。
**4 人目: 自作 node resident (独自値プリミティブ)** — 実験 12b がそのまま実証: 外部 module が
`impl @ext.NodeExt for HexArg` (eval で token_at → Binding::new → next 継続 or Held) +
`@factory.ext_node` で Node 化 + structural 合成の enum_value_node 相当も自組みできた。
**追加で必要になった公開面はゼロ** (§8.10.3 の表で完結)。

#### 8.10.5 spec 側の対応 (課題 4) と段階戦略 (課題 3)

- **DR 化が必要**: open node 契約 (DR-110 §3-2) は「engine の内蔵機構」としての規定で、
  公開拡張 ABI の規範 (NodeExt の必須/optional メソッド、Branch 3 値の安定性契約、Resume の
  保持禁止、Ctx 観測 3 面、consumes_zero_tokens の宣言義務) を言語非依存に書いた DR が無い。
  **DR-119 素案「open node 拡張 ABI」**として起草する (DR-110 の改訂でなく新規 — DR-110 は
  パッケージング構成の DR で、ABI 意味論は別関心)。descriptor (DR-107) への `role:"node"`
  行追加もこの DR の関心 (現 7 role に node が無い)
- **段階戦略は不要になった** (課題 3 への答え): §8.10.3 の公開面は mark 系 (effect/
  deprecation/failure) も含めて全部同じパターン (構築子公開) で揃い、削って得る単純化が無い。
  v1 で全面公開する

#### 8.10.6 M2c checklist (第 5 版差分 — §8.8.4/8.8.5 への上書き)

- [ ] abi に Branch / (Binding / Candidate / ParseError の private-field 化 + 構築子) を追加
- [ ] extension に Ctx 窓 / Resume typealias / NodeExt / MatcherExt / InstallerExt /
      apply_piece_filters 系ヘルパ (第 4 版で internal 送りにした trait を戻す)
- [ ] `src/kuu-node/` (仮名、public) 新設: structural 4 + ext_node + arg factory 14 +
      `pub typealias Node`。第 4 版の「factory 非公開化」は撤回
- [ ] builtins は分割せず 1 package のまま (import = abi + extension + kuu-node)。
      node/installer/matcher residents を公開 trait + kuu-node 構築子で書き直し —
      internal 直 import が消えることが受け入れ判定
- [ ] 中間状態からの継続: §8.8.5 の表は有効 (abi/internal 移動は生きる)。第 4 版で
      「internal/residents へ移動」とした作業は未着手なら**スキップ** (不要になった)
- [ ] 統括報告: residents 書き直しで公開面に不足が出た箇所 (= §8.10.1 の列挙漏れ) の一覧

### 8.11 第 5.1 版追補 — InstallerExt の居住は kuu-node (extension 居住は cycle で不成立)

> M2c 指摘 (2026-07-24) の追認: InstallerExt を extension 定義にすると、apply の引数
> (InstallBuilder) と植え付け対象 (Node) の名指し面が kuu-node (internal より下流) にしか
> 無く、extension (internal より上流) から参照すると循環する。**第 5 版 §8.10.3 の
> 「InstallerExt も成立 / BuilderView 不要は実装判断」は誤り — 実験 12 は NodeExt / factory
> 経路のみで installer 経路を含んでおらず、未検証の一般化だった** (訂正済み)。

#### 8.11.1 実験 13 (probe green) — InstallerExt は kuu-node 居住 + closure 注入

- **InstallerExt trait は kuu-node (factory) 居住**: kuu-node は internal を import する
  唯一の public 層で、`Node` / `InstallBuilder` を typealias + 窓 fn で名指しできる。
  installer は「Node を植える」役なので Node の見える層に住むのが依存構造上も意味論上も
  自然 (team-lead 候補 (i) の採用)
- **internal の lowering は trait を知らない**: kuu-node の `lower_with(builder, installers)`
  が installer 束を `Array[(InstallBuilder) -> Unit]` の **closure 列へ変換して** internal の
  `run_installers(builder, steps)` に渡す — internal→kuu-node の import は発生しない
  (実験 11 の「adapter を internal 側に」と同じ向きの解決を、trait 変換でなく closure 化で
  実現。internal は関数型しか見ない)
- **installer の登録面も kuu-node** (`NodeRegistry` 相当): extension の Registry には
  InstallerExt が見えないため、installer 束は kuu-node 側の登録面が持ち、玄関 (kuu) が
  extension Registry と合流させる。参照実装では「Registry を extension / kuu-node の
  2 段にするか、kuu-node が extension Registry を包む 1 個の合成 Registry を公開するか」は
  実装判断 (どちらも probe パターン内 — 玄関は両層 import 可)
- **外部 installer 拡張の全経路 green** (実験 13b): 外部 module が
  `impl @factory.InstallerExt`、`builder_definition_names(builder)` (窓 fn) で観測、
  `builder_plant(builder, seq([exact, bind]))` で structural 合成を植える — moon check green。
  観測が**窓 fn 形** (dot-call でなく自由関数) になるのは外部が internal 型へメソッド呼び出し
  できない制約 (実験 5/13 で再確認) の帰結

#### 8.11.2 trait 居住の最終形 — 「Node を運ぶか」で 2 層に割れる

| 居住 | trait | 判定 |
|---|---|---|
| extension | TypeExt / CompleterExt / AccumulatorExt / CollectorExt / EntityExt / CapabilityExt / **NodeExt / MatcherExt** | シグネチャが abi + extension 型で閉じる (NodeExt/MatcherExt は Branch/Ctx が abi/extension に降りたため成立 — §8.10) |
| kuu-node | **InstallerExt** | シグネチャが Node / InstallBuilder (internal 型の名指し) を運ぶ唯一の trait |

設計の骨 (構築と観測の公開 / 分解は評価器の専権) は不変 — InstallerExt の居住が 1 層
下がるだけ。**DR-119 への影響なし** (DR-119 は言語非依存の契約規範で、trait の package
居住は言語実装形 = 本 findings の関心。DR-119 §4 の構築面・§8 の role 追加はそのまま)。

#### 8.11.3 M2c checklist 差分 (§8.10.6 への追補)

- [ ] InstallerExt を extension でなく kuu-node に定義 (Registry の installer 登録面も同層。
      extension Registry との合流形は実装判断 — 玄関で合成 or kuu-node が包む)
- [ ] internal の lowering は installer 束を closure 列 (`Array[(InstallBuilder) -> Unit]`)
      で受ける形に変更 (installer fixpoint ループの引数型変更のみ、意味論不変)
- [ ] InstallBuilder / DefsView の観測・植え付け窓 fn (+ typealias) を kuu-node に設置 —
      集合は builtins installer 15 本の書き直しが確定する (§8.5 の機械検出パターン)
...
- 実験 12 (第 5 版): factory package (internal import + 構築子 + `pub typealias @eng.Node as Node`)
  で外部の NodeExt impl / structural 自組み / typealias 型注釈が全 green。外部の internal 型名
  直書きは `Package "eng" not found` (typealias が名指し面を提供する根拠)
- 実験 13 (第 5.1 版): InstallerExt を factory (kuu-node 相当) に定義、installer 束を
  closure 列で internal の run_installers へ注入、InstallBuilder の窓 fn
  (builder_definition_names / builder_plant) 経由で外部 installer 拡張が観測・植え付け —
  moon check green (mbtprobe `ran 8 tasks` / consumer `ran 2 tasks`)。外部から internal 型への
  dot-call は `Cannot call method of type @probe/x/internal/eng.InstallBuilder: package ... is
  not imported` (窓 fn 形が必須の根拠)
## 関連
