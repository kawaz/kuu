...
> typealias を公開。実験 12 で 4 顧客 (自作 node resident 込み) green**。
> §2 の型リストは §8.10.3 の表が最終正。
...
| `InstallerExt` | extension | InstallBuilder / DefsView も同じ「観測窓 + 構築子」パターン: builder の観測 (definition 系 getter) と植え付け (`InstallOutput::new(Array[Node])` — Node は typealias 名指しで書ける) が公開型だけで書けるようになった (第 3 版で不可能だった理由 = Node が書けない、が typealias で消えた)。**BuilderView の発明も不要** — 既存 InstallBuilder / DefsView を extension の観測窓 struct として定義し直すか internal のまま typealias するかは実装判断 (どちらも実験 9/12 の green パターン内) |
