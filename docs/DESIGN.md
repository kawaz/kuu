...
`filters` / `pre_filters` / `post_filters` は 2 つの形を取る (multiple / variant と同じ string 短縮形 | object 詳細形のイディオム):
...
- env 値は string として pieceProcessor (pre_filters → parse → post_filters) を通る。multiple 要素なら separator 分割も効く (CLI 入力と同じ手順、DR-049)
