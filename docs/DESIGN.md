...
| `installers` | 特殊語彙の展開装置 (糖衣展開 + 実行時能力の植え付け、DR-042) | `long`, `short`, `env`, `type:"dd"` 等の特殊語彙 |
...
「最長一致」をプリミティブな規則として持たない。長い経路が短い経路を抑える挙動は、入力を全消費する経路のうち短いものが「未消費トークンが残るため失敗」となる結果として創発するだけで、規則ではない。

経路の同一性は**観測可能な束縛列**で判定する (探索の道順が違っても束縛列が同一なら 1 本と数える。DR-038)。
...
- **先食い**: スコープの positional 面の消費点で、トリガ語彙 (greedy に読めるトークン + 宣言済み未発火の dd マーカー) に一致するトークンは素通し読みの枝を生成しない。positional 面は多孔質 (トークン間に greedy が割り込める)、発火した greedy の内部消費は一体 (後続トークンを無条件に raw 消費、getopt 同型)
- **prefix ガードなし**: 未定義の `-x` は素通しで positional に落ちる。形が option 風であることを理由に拒否しない (拒否したい場合は方言で opt-in)
- **dashdash**: dd は再スコープ化の糖衣。スコープ境界が greedy 面を断つこと (DR-042) で option 抑制が出る
...
| **matcher** | installer が評価器に登録するトークン読みの実行時能力 (DR-041) |
| **読み (reading)** | matcher が 1 トークンに対して列挙する解釈候補。各読みは path-search の枝 (DR-041) |
| **先食い** | greedy がスコープの positional 面に優先し、素通し読みの枝を生成させない規則 (DR-041) |
