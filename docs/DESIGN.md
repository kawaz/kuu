...
### 9.4 group_rules は作らない (DR-012)

「グループ全体に対するルール」を別場所 (`group_rules` 等) に書く設計はしない。各要素属性で表現できる範囲に限定。

### 9.5 制約の評価 (DR-047)

required / requires / exclusive_group は**遅延述語**であり、完全解決経路の成立条件として評価される (事後検証層は無い)。詳細は §15.9。
...
| `installers` | 特殊語彙の展開装置 (糖衣展開 + 実行時能力の植え付け、DR-042) | `long`, `short`, `env`, `type:"dd"`, `commands[]`, `global`, `inherit`, `repeat`, `multiple` 等の特殊語彙 |
...
### 14.3 visibility / deprecated

ヘルプ/補完での表示制御のためのフィールド予約。

```json
"hidden": true       // 予約: ヘルプ・補完での非表示の意図
"deprecated": true   // 予約: 使用時の非推奨表明の意図
```

現状は AtomicAST でフィールド名を boolean で予約するのみ。ヘルプ表示・補完露出・警告メッセージなどの具体的な挙動仕様は本仕様では確定しない (別 DR で確定する)。パース挙動 (CLI からの受理可否) には影響しない。
...
- **同一トリガは最小スコープ優先**: global コピー等で衝突したら内側の宣言が食う (lexical 解決のパース時適用、判定は要素名でなくトリガ literal)。同一スコープ内の重複は静的 warn + 実行時 ambiguous
...
- **失敗時** (完全経路 0 本): 候補経路のいずれかで失敗時アクション持ち要素が selected だったなら、保持 Error の表示に代えて発火する (help なら help 表示)。複数観測されたら **argv 上の消費位置が最小のもの** (先勝ち、効果列 = DR-045 の順序。DR-015 のあと勝ち mutation と同じ時計)。枝の深さ・走査順は座標にしない

失敗時アクションは汎用属性で、`type: "help"` プリセットが同梱する (属性のフィールド名は §13.9 で未予約)。「候補経路で selected」の精密定義 (partial 経路の範囲) は本仕様では未確定 (issue `2026-07-03-failure-action-selected-scope`)。
