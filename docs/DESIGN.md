...
- **「読める」の精密化 (DR-097)**: 先食み・早閉じ抑制が共有する述語「読める」は、トリガ一致ではなく**その greedy 読みを経由する完全経路が存在すること (読みの成立)** を指す。値照合失敗・後続トークン枯渇などで完全経路に 1 本も届かない消費点は読みゼロと同じ扱いになり、素通し枝 (positional raw 消費) が生きる。生きた素通しから完全経路が立てば採用され、greedy 側の held Error は他ルートが通るぶん捨てられる (DR-037)
