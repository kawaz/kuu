...
表示メタ (`help` / `help_long` / `help_epilog` / `display_name` / `value_name` / グループ・順序語彙、§14.6) の「AtomicAST 非搬送」(DR-046 §3) は「**lowered 産物・評価器へ運ばない** (パース挙動に影響しない)」の意であり、宣言層 (wire form、DR-063 §1) には inert 属性として載る (DR-112 §1)。help query (§15.15) の入力は宣言層なので矛盾しない。帰結として lowered 断面だけ持っている実装は help を出せない。`help` の型は string。多言語対応は UsefulAST 層 (各言語 DX) の関心であり、AtomicAST レベルではサポートしない。
...
**糖衣プリセット**: `flag` / `count` / `count_or_set` / `command` / `help` / `help_all` / `help_category` / `dd` / `tty` 等
...
- `help` = help 機構の内部セルへの link + 固定 true 供給 (§14.1)。`help_all` (hidden 込み全表示の意図メタ) / `help_category` (string カテゴリセル + help 内部セルの同時トリガ) は同族 (DR-112 §7)
...
失敗時アクション属性は `on_failure` として確定済み (§15.10 / DR-112 §8 — 専用 installer `on_failure` が所有する能力宣言型)。「help installer (lowering 装置)」は存在しない — help は query であり (§15.15、DR-112 §1)、表示メタ語彙の所有座席は純所有 installer `help_meta` が担う (lowering 寄与ゼロ、DR-112 §9)。

installer が宣言語彙に関わる形は 2 種類ある (DR-056): **所有** (lowering 責務、1 語彙 1 所有者で排他 — `help_meta` のような lowering 寄与ゼロの純所有もこの形) と**参照** (advisory read、自由 — help query / 補完生成が alias / hidden / deprecated / long を読んで表示・補完データを作る等)。参照読みの成果はパースの観測挙動 (効果列) に影響してはならない (副次成果物の構築のみ)。参照が許されるのは宣言層であって他 installer の lowered 産物ではない。
...
### 14.1 help — 内部セルモデルとプリセット 3 種 (DR-112 §7)

```json
{"name": "help", "type": "help", "long": true, "short": "h", "global": true}
```

**内部セルモデル**: help の値セル実体は help 系要素のどれでもなく、**help 機構が管理する内部セル**である。各 help 系要素 (`type: "help"` / `"help_all"` / `"help_category"`) はそこへ link される:

- どのサブコマンドスコープで発火しても同じセルに立つ (全体単一セル)。global + link の合成 (既存機構) を「help 機構管理の内部セルへの合流」として実現する
- 内部セルは `#` 予約名前空間 (DR-046 §4) の実装細部であり、wire にも result にも直接現れない。**result への露出は各 help 系要素自身の name (export_key) 経由** — link の既存意味論 (値セル共有、露出は各入口の export_key) の素直な適用。`{"name": "help", "type": "help"}` が発火すれば result に `help: true`、`{"name": "help_category", "type": "help_category"}` なら発火カテゴリ文字列が `help_category` キーに出る
- パーサ完了時の出力切替は内部セルの真偽の観測。`fired_action` (DR-053 §2) は発火した具体要素の name のまま不変
- help_category のみ存在 (type:help 露出なし) でも help の bool 充足自体は成立する (内部セルに立つ)

**プリセット 3 種** (いずれも bool を土台にする flag 同族の糖衣プリセット。lowering は LOWERING §A.5):

- **`type: "help"`**: 入口 lowering は long / short と同型。値セルは内部セルへの link、発火は固定 true 供給。`help_on_failure` (bool、既定 true) を type config 糖衣として同梱 — 汎用属性 `on_failure` へ全展開される (§15.10 / DR-112 §8)。パース失敗時 (完全経路 0 本) も、候補経路で help が selected だったなら保持 Error に代えて help を表示する (失敗時アクション、DR-048 / §15.10)
- **`type: "help_all"`**: type:help と同じ preset 族 (入口・内部セル link・help_on_failure 同梱まで同一)。異なるのは**意図メタ**のみ — 「hidden 込み全表示」の要求をアプリ / レンダラに伝える (DR-058 §1 の「--help-all で hidden も表示」の定義側座席)。help model は hidden をメタとして常に残す (§15.15) ため、レンダラが help_all 発火を見て hidden 表示を選ぶだけで足りる
- **`type: "help_category"`**: 内部的には string の全体セル 1 つ (内部セルモデルと同型、link 合成)。**発火時に help の内部 bool セルもトリガする** (bool セルへの固定 true 供給を同梱) — 「カテゴリ指定 = help 表示要求」が 1 発火で両セルに立つ。bool 枝 (`--help` 裸) と string 枝 (`--help net`) は **`or` で type:help と出し分け**る — 引数有無の曖昧さの解決を or の経路成立に乗せる (optional 値スロットの特殊規則は持たない)。`values` で指定可能カテゴリを制限できる (value-enum、§5.3 — カテゴリの名前空間は §14.6 のグループ名と同一)。`--help-command` のような引数なし入口は variant DSL の固定値 set (`[":set:command"]`) で組める。複数 category 指定は string 全体セルの last-wins (DR-015) が既定 (multiple 宣言との合成は v1 で規定しない)

**lint warn の条件 (発火の観測手段)**: warn の判定は name (export_key) 露出の有無で決まる。help_all / help_category はそれ自体が「help を引く入口 + 結果面の露出」を兼ねられるため、type:help 不在でも name 露出があれば warn 不要。warn になるのは「help 系要素はあるが、どれも結果面に露出していない = 発火をアプリが観測する手段が無い」構成のみ (definition-error にはしない)。
...
- ただのフラグ
- 結果オブジェクトの `result.version` を見てアプリがバージョン出力 — version への反応は成功・失敗ともアプリの仕事 (help も同型: kuu が出力まで肩代わりするのではなく、model / レンダラを部品として提供しアプリが接続する、DR-112 §10)
- AST にバージョン文字列を持たせない (help model にも載らない)
- パース失敗時には何も出ない (結果がアプリに渡らないため)。失敗時にも version を出したいアプリは `on_failure: true` を opt-in する (DR-048 / §15.10 / DR-112 §8)
...
- help では canonical の行に併記 (独立一覧しない)。表示活用は help query / 補完生成の参照 (§13.9 の所有/参照)

### 14.6 help 表示メタ語彙 (DR-112 §4〜§6/§9)

宣言層の inert 属性群。パース挙動に影響せず、help query (§15.15) が参照で読む。語彙の所有座席は純所有 installer **`help_meta`** (canonical セット、lowering 寄与ゼロ — 宣言層に inert のまま残し何も植えない。本節の定義時検査は parse_definition 寄与として実装する)。所有語彙: `help` / `help_long` / `help_epilog` / `display_name` / `value_name` / `help_group_name` / `help_group_title` / `help_group_description` / `help_group_order` / `help_order` / `help_after`。`hidden` / `deprecated` の所有座席は既存の空白のまま (installer descriptor 実体化時に一括で埋める、DR-112 §9)。
...
6. **同じグループ名に対する別設定の重複宣言は definition-error** (kind: `invalid-range`)。同一設定の再宣言は冪等で合法
...
- **糖衣 = `help_on_failure` (bool、既定 true)**: type:help 系プリセット (help / help_all / help_category) が同梱する要素属性。type config (プリセットの属性プリセット展開、DR-076 の枠) で汎用属性 `on_failure` へ全展開される。`help_on_failure: false` で失敗時発火を切れる。type:help 系以外の要素への `help_on_failure` は definition-error (kind: `invalid-range`) — 糖衣の座席はプリセットに閉じる
...
本節の「実装が準拠を名乗る条件」と、spec バンドル自体が v1.0.0 として確定する条件 (5 プロファイル全 green を要求、DR-112 §11 で 4 → 5 に改訂) は別軸 (DR-068/DR-069/DR-108)。

### 15.15 help クエリ (DR-112)

**「help installer」は存在しない — help は query である**。completion と同じ分解 (DR-060/104 の前例): 表示メタは宣言層の inert 属性 (§14.6)、`type:"help"` 系は types 側のプリセット (§14.1)、失敗時アクションは汎用属性 `on_failure` (§15.10)、表示データの組み立ては complete query と同格の**別クエリ**、レンダラ (文言・幅・折返し・色・ソート・翻訳・ページング) は層 2 (DX 層) で射程外。
...
- `depth` は既定で選択スコープの 1 層分 (サブコマンドは一覧 entry のみで中身は載せない)。`"all"` で全層再帰 (man 生成等の一括消費者向け)。中間の数値 depth は持たない (追加互換で足せる)
- `category` は §14.6 のグループ名を指し、model の entries を該当グループ所属のもの (+ 当該グループ宣言エントリ) に絞る。`type: "help_category"` の発火値を受けてアプリがカテゴリ別 help を組む入口。存在しないグループ名は `absent-ref`
...
**help model** (DR-109 柱 4 の「semantic sections」の具体形)。全フィールドが素材であり、文言・整形・並べ替えを含まない (DR-053 の「素材はフィールド、文言はレンダラ」の一貫適用):
...
- **`command_path`**: ルート定義要素の name (あれば) を先頭に、path の連鎖を続けた列。ルートに name が無い定義では path のみ — プログラム名 ($0) は args 非包含 (§0.1) で定義に存在しないため、usage 行の `prog` はレンダラ / 呼び出し側の供給
- **`hidden` はメタとして残し、既定除外はレンダラの policy** (complete API と同型 = DR-104 の `meta.hidden`)。model が hidden を落とすと「--help-all で hidden も表示」をレンダラが作れなくなる
- **alias は canonical entry に `alias_spellings` として併記** (§14.5 の構造化)。variant 入口 (`--no-ssl` 等) は `spellings` に全 long 入口を列挙する形で素材化 (variant DSL の解釈は long 属性の値読みで得られる — 宣言層読みの範囲内)
- **`default` / `env` / `required` / `multiple` は注記素材** (`[default: 8080]` / `[env: PORT]` 相当をレンダラが組むための値)。deprecated の「use X instead」素材は canonical 自動導出 (§14.4)
- **usage は構造素材のみ、一行文字列は含めない**。素材は「positional 進行の要約 + has_options / has_subcommands / has_dd」に留め、一行化・ネスト構造の丸めはレンダラの判断。この粒度は意図的な妥協 — 忠実な usage を組みたいレンダラは definition を直接読めば済む (wire form が既にその情報を持つ)。usage tree の将来拡張は追加互換
- **version 文字列は載らない** (§14.2 の帰結)
- **`depth: "all"` の再帰形**: 各 command entry に `scope` フィールド (その command を選択スコープとした help model の再帰埋め込み) を足す
- **entries の順序は規範**: `options` / `commands` は「宣言順に §14.6 の並べ替え規則を適用した後の順序」を保存する列であり、conformance は順序込みで比較する (CONFORMANCE §5)。`positionals` は定義順 (= 消費構造順) 固定。alphabetical 等への並べ替えはレンダラ policy
- **フォールバックは model に持ち込まない**: `help` / `help_long` は未設定側を省略しそのまま載せる

kuu-cli には `kuu help <definition.json> [--path sub...]` として写る (VISION §3)。極小バンドルモード (DR-109 §6) では、アプリの help 表示自体を kuu-cli への問い合わせ (help query の JSON 出力 + アプリ側の薄いレンダラ) で賄える。
...
| **失敗時アクション** | 完全経路 0 本の失敗時、候補経路で selected なら保持 Error に代えて発火する表示動作 (help 等)。early-exit は存在しない (DR-048)。宣言は `on_failure` 属性 (DR-112 §8) |
| **help query** | 宣言層 → help model の純関数クエリ。パース実行なし・args 不要 (§15.15、DR-112) |
| **help model** | help query の出力。文言・整形を含まない表示素材の構造 (semantic sections の具体形、DR-109 柱 4 / DR-112 §3) |
| **help_meta installer** | 表示メタ語彙の純所有 installer (lowering 寄与ゼロ、unknown-vocab 検査上の所有者 + 定義時検査、DR-112 §9) |
| **グループ宣言エントリ** | `options[]` に置く、グループ属性だけを持つ entry。グループの表示順とメタが一箇所で完結する (§14.6、DR-112 §5) |
