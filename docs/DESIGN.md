...
目的語の充足判定は「値の有無 (default 込み)」だが、**目的語が bool 型 (flag preset 含む) の場合は「解決後の値が true であること」(値源不問 — cli / env / config / inherit / default のどれ経由でも true なら充足)**。bool は preset の暗黙 default:false を常に持つため、値の有無判定だと bool 目的語の requires が vacuous に充足してしまう (DR-047 明確化 2026-07-09)。
...
| `requires` | 混合 | トリガ側 = committed、目的語側 = 値の有無 (**bool 型目的語は「解決後の値が true」、値源不問** — DR-047 明確化 2026-07-09) |
