...
- **同一スコープに複数の config_file 要素を宣言してよく、席埋め規則は単体も複数も 1 本** (DR-133)。各要素が供給するパスを**宣言順に並べた 1 本のパス列**を作り (`multiple` の要素は自分の位置に供給順で inline 展開、どの席からもパスが供給されない要素は列に参加しない)、列の各パスを順に provider へ渡し (1 パス → 1 オブジェクトの契約のまま列の長さだけ呼ぶ)、読めたオブジェクトを**トップレベルキー置換**で畳んだ 1 個が config 席に座る。後のファイルのトップキーが前を置き換え、後に無いトップキーは前の値が残る (= 重ね)。**深いマージはしない** (需要は合成済みオブジェクトを返す provider の関心)。上記の committed Error / default 由来不在の黙認は**各パス独立**に判定する。単体要素は列長 1 の縮退形で、排他的なファイル選択は 1 要素の値源ラダーで書き分ける
