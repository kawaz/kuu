...
- **lowered 断面の表記** (lowering 段階別 fixture の期待値用) は面構造 `{greedy, positionals, entities}` / matcher `{matcher: kind, entries}` / 効果記述子 (§7.4、DR-045)。比較は構造骨格 + matcher 種別・エントリ表の緩比較 (LOWERING §C.5)
