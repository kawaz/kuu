...
installer / registry 住人は自身を説明する **descriptor** を持つ (DR-061): installer は所有語彙 (`owns`、排他 = DR-042 不変則③)・観測語彙 (`observes`、表示・補完等の副次成果物にのみ影響)・自身が読む config キーを宣言する。宣言された所有集合の和が、直列化された定義上の語彙の正当性判定 (誰も所有しない語彙 = DR-054 の unknown-vocab Error) と completeness 検査の判定入力になる。observes は lint / diagnose の素材であり実行時強制はしない。
...
JSON Schema の実体化は残作業 (F-042 / F-048)。
