...
- 基数 prefix (`0x`/`0o`/`0b`) と hex float (`0x1.8p3`) は **`number_allow_base_prefix` の統合 opt-in** — canonical (default false) では `0x1F` も `0x1.8p3` も Error。有効時は `p`/`P` 任意 (`0xff`→255 も可)。`int` 型の判定基準 (値空間判定 `0x1.8p3`=12 可、vs 整数構文判定) は裁定確認中 — DR-074 §2 の暫定注記を参照、確定規定として扱わない。
