...
### 2.3 結果キー軸の占有が結果スコープを作る (DR-025、TRG-Q4=a)

- **解決後の結果キー (`export_key` 軸) を占有するノード** = 結果スコープ (結果オブジェクトのキー) を作る
- **占有しないノード** = 透過 (値の畳み方だけ効かせて値を親に流す)

children の有無でも name の有無でもなく **結果キー軸を占有するか**で結果スコープが決まる。
`name` は各軸へのデフォルト供給源にすぎない (DR-136 §1) ので、`{"name": "box", "export_key": null}`
...
- 基数 prefix (`0x`/`0o`/`0b`) と hex float (`0x1.8p3`) は **`number_allow_base_prefix` の統合 opt-in** — canonical (default false) では `0x1F` も `0x1.8p3` も Reject。有効時は `p`/`P` 任意 (`0xff`→255 も可)。`int` 型は値空間判定 (DR-075) なので opt-in 時 `0x1.8p3`=12 は整数値として受理、`0x1.8p0`=1.5 は fractional 値として int_round に従う (10 進の `2.5` と同じ規則)。
- **inf**: `float` 型のみ受理 (`inf`/`infinity`/`Inf` を case-insensitive で kuu 固定)、`number` 型では Reject。言語 DX が float の inf 非対応を選ぶのは許容。**nan**: 両型 Reject (opt-in も置かない)。**型 suffix (`1.0f`/`100n`) は非採用** (float/double は type で、任意精度は言語実装側の拡張型の領分 — 下記「整数の保証精度」参照)。`,` 系桁区切りは方言、単位 suffix は型の領分 (duration/size 等)。
...
**bool** の canonical 受理語彙 (DR-074、config キー名は DR-100 で `bool_` prefix 化): `bool_true_values` default `["true","1"]` / `bool_false_values` default `["false","0",""]` / `bool_case_insensitive` default `true`。空文字 `""` は false 側 (env の `FLAG=` 対応、`bool_false_values` の要素として表現)。`yes`/`no`/`on`/`off` は canonical 外 (Norway problem 回避、config で opt-in 追加可)。値空間外の bool 文字列は Reject (値へ変換しない)。**bool↔number**: 文字列 `"1"`/`"0"` の parse (String→bool) は可だが、number 型の値 `1` からの型変換 (config native number→bool) は Error (DR-050 の非強制規定は不変)。
...
`value:` と `default:` は**位置に依らず別位相**である:
...
`short_combine` (bool、既定 `true`): 複数 short オプションを 1 トークンに束ねるクラスタ読み (`-ab` = `-a -b`) を枝として列挙するか。`false` はクラスタ読みのみを禁止し、単独発火と値の直接付着 (`-p80`) には影響しない — 値付着の制限は独立した方言パラメータ `short_attached_value` (DR-096 §2) の管掌であり、`short_combine` はあくまで複数 entry が同一トークンを分け合う行為だけを指す (DR-014「-abc の結合許可」)。gcc の `-O2` / `-Wall` 型 (値付着はするがクラスタリングはしない) を表す方言パラメータ。`short_attached_value` はクラスタ内でも同一に効く — ダイヤルは entry の付着読み / space 形読みの生成だけを制御し、クラスタ内の位置は条件にならない (DR-096 §3.1)。**クラスタ成立は全参加者の conjunction**: あるクラスタ読み枝が立つのは、その枝に参加する全 short entry が同一 `(short_prefix, short_combine:true)` の組に属する場合に限る。要素単位 override で一部の entry だけ `short_combine:false` にすると、その entry を含めるクラスタ枝は生成されず、`(short_prefix, short_combine:true)` の組に属する entry どうしのみで別クラスタ読みが立つ (要素単位 override × cluster の帰結、fixture: `fixtures/matcher-readings/short-combine-off.json`)。
...
### 11.1 スコープは結果キー軸の占有で自動 (DR-025, DR-033、TRG-Q4=a)

解決後の結果キー (`export_key` 軸) を占有するノードが結果スコープ = lexical スコープを作る。
children の有無も name の有無も無関係 (`export_key: null` は透過、name 無し + 明示 `export_key` は
スコープを作る)。透けるのは名前解決の面だけで、トリガ照合・入場・背骨といったパース経路の面は不変。
...
- **1 スコープ 1 config_file — 並置は definition-error `invalid-range`** (関与要素ごとに 1 件、DR-133 §1)。config 席はスコープに 1 つ (§11.4) なので、2 つ以上の config_file 要素は同じ 1 席への重複宣言になる。**重ねの正規形は 1 要素の `multiple`** (`{"type":"config_file","multiple":"append","default":["/etc/app.toml","~/.app.toml"]}`) — 綴り違いの入口は `alias`、由来違いの入口は元からラダーの 3 席、既定パス列は配列 `default`、分割指定は `borrow` で、並置の動機はすべて 1 要素に収まる
- **席埋め規則は「パス列 → 1 オブジェクト」の 1 本** (DR-133 §2)。その要素が供給するパスを**供給順**に並べ (単値は列長 1 の縮退形)、列の各パスを順に provider へ渡し (1 パス → 1 オブジェクトの契約のまま列の長さだけ呼ぶ)、読めたオブジェクトを**トップレベルキー置換**で畳んだ 1 個が config 席に座る。後のファイルのトップキーが前を置き換え、後に無いトップキーは前の値が残る (= 重ね)。**深いマージはしない** (需要は合成済みオブジェクトを返す provider の関心)。どの席からもパスが供給されなければ列は空で config 席も空 (エラーではない)。上記の committed Error / default 由来不在の黙認は**各パス独立**に判定する。排他的なファイル選択は `multiple` を付けない 1 要素で書き分ける (ラダーが「どれか 1 つ」を担う)
- **committed 判定は link を透過する** (DR-133 §4.1)。値を運んだ経路ではなく**値の源席の cli / env 性**で判定するので、link 経由で CLI 供給されたパスも committed (読めなければ Error)。ただし `sources` の観測タグは `link` のまま (DR-121 §4) — 由来を辿るのは committed 判定 (内部) だけの別軸
- **config_file 要素は通常要素であり、結果面でも特別扱いしない** (DR-135)。露出キーを持てば `result` / `sources` に**確定したパス**が座り (`multiple` なら供給順のパス列、未供給の座は `null`)、cli / link 由来の発火は `effects` にも載る。座るのは供給されたパスであって読めたパスではない (読めたかは fold の関心)。sources のタグは自分のラダー席 (`cli` / `env` / `default` / `link`) で、循環禁止により `config` タグは立たない。露出キー衝突検査には**占有子として参加**する (§15.5 / DR-120 §4)。結果に出したくない定義は `export_key: null` (透過、値の伝搬は止まらないので config 席の充填は働く) か nameless で書く
...
- `type: "config_file"` の要素 — 値 (パス) を持つ通常要素なので占有側 (§14.3、DR-135)
