...
方言バリエーションは value_parser クロージャの量産でなく **configurable factory** で表現する (DR-061): registry 登録値を `{"name": "<factory名>", "config": {...}}` の形にし、動作差分を純データ config で与える (`number_parser`: `number_thousand_sep` default `["_"]` / `number_allow_base_prefix` default `false` / `number_leading_zero` default `"decimal"`。`bool_parser`: `true_values` default `["true","1"]` / `false_values` default `["false","0",""]` / `case_insensitive` default `true`。`int_parser` (= `builtin/int_parser`、DR-094): `int_round` default `"error"` (非整数値の丸めモード 10 種、DR-075)。DR-074 §4)。**canonical default = factory の default config** であり、標準層 opt-in・方言は config キーの列挙になる。config は純データなので定義とともにシリアライズされ、方言構成の再現性が wire 上で担保される。factory config が調整するのは parse 相 (String → T) の内部のみで、相の間の変換・検証は filter の領分 (§8.3、DR-061 §5 の「相」線引き)。
...
`long_eq_sep` (`"require"｜"allow"｜"deny"`、既定 `"allow"`、DR-096 §1): long の eq 分割形 (`key=value`) と space 分割形 (`key value`) の入口をダイヤルする。`"require"` は eq 分割形のみを受理し別引数での値供給を拒否 (DR-091 §3)、`"allow"` は両方を受理、`"deny"` は eq 分割 matcher を生成せず space 分割形のみを受理する。`long_prefix: ""` (空 prefix) は無条件に合法 — bare な名前一致トリガ (`height 168.5` 型) が素の operand と衝突しうるかはアプリ毎の事情であり、衝突する定義では読み列挙が全解釈を Ambiguous として列挙する (DR-053)。`"require"` は衝突を定義側で排除したい場合の解決手段の一つ (eq 必須なら `=` を含むトークンだけが long 候補) であって合法性の条件ではない (DR-096 §3.3)。3 値 enum のため「eq 必須かつ eq 禁止」のような矛盾する組合せはそもそも構文的に表現できない (illegal states unrepresentable、DR-096 §1)。

`short_attached_value` (`"require"｜"allow"｜"deny"`、既定 `"allow"`、DR-096 §2): 値持ち short の付着形 (`-O2`) と space 分割形 (`-O 2`) の入口をダイヤルする。`"require"` は付着形のみを受理し space 分割形の読み枝が立たない (gcc/clang の `-O` / `-W` 型)、`"allow"` は両方を受理 (DR-041 §4 の値スロット一般規則)、`"deny"` は付着読みの枝を生成せず space 分割形のみを受理する。クラスタ末尾の値スロット entry も単独発火と同じ値スロット一般規則に従い、付着に加えて次トークンからの space 分割形供給を受けられる (`-abp 80` の `p` が `"80"` を取る) — 値取得の 2 形モデルは発火文脈 (単独 / クラスタ末尾) に依存せず、`short_attached_value` はこの space 形も同一にダイヤルする (DR-096 §3.1)。値スロットを持たない要素 (flag/count) にはダイヤルが届いても inert (DR-096 §3.2)。gcc/clang は per-option でこのダイヤルが分かれる (`-O2`/`-Wall` は attach-only、`-I`/`-l` は両対応) ため、scope 既定とは別に要素単位 config override (DR-049 §4 と同機構) で個々のオプションに指定する。
...
- **multiple 経路の後**: 累積結果に対する最終変換
  - collector / cell_filters: `T[] → U` (to_set、to_map 等、累積後に効く)

両者は位置が違うので自然な順序で合成 (parse 後 → 各 piece の value_filters、accumulator 後 → cell_filters)。
...
どの chain を通るかは値源席の規定 (DR-049 / DR-050) の管轄。なお `piece_filters`
(String→String) は消費した raw string に乗るため、値トークンを消費しない cell 操作の発火には
そもそも走る場面が無く (帰結は同じ、理由が異なる)、`cell_filters` は発火単位でなく累積後の
最終値に乗るため本規定の対象外。
...
`value_filters` / `piece_filters` / `cell_filters` は 2 つの形を取る (multiple / variant と同じ string 短縮形 | object 詳細形のイディオム):
...
`required: true` + `default` は常に充足する — required の実質は「値源を持たない要素への明示強制」と「結果に必ず値がある」という型保証であり、none でも崩れない (none の保証は「発火した事実が必ずある」)。none には default / env 等の値源が無いため、none 要素の充足経路は CLI 上の発火のみ。グループ的必須は or + required:
...
- **目的語が bool 型** (flag preset 含む): 解決後の値が true であること (値源不問 — cli / env / config / inherit / default のどれ経由でも true なら充足)。bool は preset の暗黙 default:false を常に持つため、値の有無判定だと bool 目的語の requires が vacuous に充足してしまう (DR-047 明確化 2026-07-09、bool 型の充足定義として DR-093 の型委譲に統合)
...
| `filters` | 純粋 FilterChain (collector も含む) | `value_filters`, `piece_filters`, `cell_filters` |
...
wire の well-formedness は 3 層 (構文 = DR-067 の invariant / 語彙 = descriptor 所有集合 / 参照 = DR-054 の解決検査) で判定する。JSON Schema と spec バージョンの lifecycle は DR-068 (確定版 v1.0.0 の発行 = 参照実装が fixture green のフェーズ 3 完了時、それまでドラフト期)。Schema ファイルのドラフト書き出しは残作業。
...
- **遅延述語**: 経路全体の最終状態を見る検証 (required / requires / exclusive_group / conflicts_with)
...
```
complete(atomic, {before, word, word_suffix?, after?}) → 候補構造
```

- `after` (カーソル後のトークン列) が与えられれば「候補採用後に after も消費して完全経路に到達できる」もので絞る (after 整合フィルタ — 全解決モデルならではの精度)
- 候補 = exact 綴り (メタ: canonical/alias・hidden・deprecated・終端ヒント word_end/continue) + 値位置の型情報 / completer 名。**素材とメタのみ返し、絞り込みポリシー (tab-tab 切替等)・置換・着地は生成器と shell の領分**
...
| **遅延述語** | 経路全体の最終状態を見る検証 (required / requires / exclusive_group / conflicts_with)。完全解決経路の成立条件 (DR-047) |
