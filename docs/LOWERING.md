...
- **出力断片は DR の疑似 JSON 記法を用いる**。lowered 断面の正式表記は DR-063 §3 で確定済み (5 面構造・
  matcher・効果記述子・templates。§C.3 参照) であり、本書の出力断片は規則説明用の疑似 JSON である:
  - `«greedy»` … その衛星が greedy 面の住人であることを示す注記 (正式表記では greedy 配列への所属で表現)
  - `{matcher: "<種別>", entries: {...}}` … 名前付きデータとしての再解釈 matcher (DR-042)。この形が DR-063 §3 の
    正式表記でもある (entries のキー語義は matcher kind が定義)
...
wire form (実装間交換形) は**宣言層のみ** (A 群糖衣適用済み + installer 語彙 inert) であり、本書 §B の lowered
出力断片は wire に載らない (宣言層から決定的 lowering で再導出可能、DR-063 §1)。
...
JSON Schema の実体化は別途 (DESIGN §15.7)。

**由来**: DR-063, DR-039, DR-041, DR-042, DESIGN §15.7 / §15.8

### C.4 記法糖衣 (A 群) は installer 不動点の前段の純構文正規化

§A の構造記法の糖衣 (裸文字列 → exact、裸配列 → seq、裸リテラル → 照合消費、values → or) は、§B の installer が
語彙を回収する **前段の純構文正規化**である。installer の不動点反復 (§C.2) に入る前に適用され、args を見ず・値源
...
- **主 oracle は効果列** (DR-038): 同一 args を与えたとき同一の結果 (効果列 = 各要素が (実体, op, operand, source,
  順序) の列) を出すことが第一の一致基準。断面の緩比較 (§C.3) と独立に観測可能な効果列は確定しているため、実装間の
  等価性は効果列で判定できる。
...
- **順列一致の property test は直列形確定前から書ける**: 「同じ効果集合を与える args 順列は同じ結果へ収束する」性質
