...
2. **conformance の実働化 + fixture 蒸留** (codex 方向性レビュー 2026-07-05 を受けた再構成 — 「追加設計でなく最小 runner の実測で前提を退役させる」):
   1. **最小 runner の早期稼働**: slice PoC の既存エンジン (167 テスト) に fixture runner を生やし、fixtures/dd/*.json を実食する。fixture 自体の誤り (期待値の書き損じ・仕様矛盾) をこの時点で検出する
   2. **lowering conformance の確定**: `query: "lower"` フォーマット + installer 単体/組合せ/全収束形の golden 断面 + 順列一致 property。DR-063 (宣言層 wire) の「受信側 lowering の担保」はここが実体
